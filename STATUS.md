...
**Side track in flight — B-054 multi-device down-sync (hydration).** New backlog scope (requirements: `docs/multi-device-sync-requirements.md`; 4 phases). **Phase 0 DONE** — jest runner wired (closes B-026); PR #82 **merged to main 2026-06-05**. **Phase 1 ENGINE BUILT this session (2026-06-05)** — `hydrateFromCloud()` in `lib/sync.ts` (events/meals/event_attachments/vet_visits/vet_visit_attachments; paginated full pull; `ON CONFLICT DO UPDATE`/`DO NOTHING`, never `INSERT OR REPLACE`, to dodge the cascade footgun), wired into `runSync` push-before-pull (FR-2), FR-9 logout local-wipe gated on the `SIGNED_OUT` auth event (`lib/db.ts` `clearLocalData` + `app/_layout.tsx`), FR-10 attachment render-from-Storage tolerance (`app/event/[id].tsx`). Pure reconcile logic in `lib/hydration.ts` (16 unit tests). **Adversarial review broke the first cut twice** → fixed: (1) FR-6 "meals immutable" is false (`updateMealIntake`/`updateMealFood` mutate in place) → new `'refresh-if-synced'` strategy propagates cross-device intake corrections without a schema change (principled `meals.updated_at` fix tracked as **B-055**, for Phase 2); (2) un-paginated pull truncated long histories at 1,000 rows → `fetchAllRows` paginates. Re-review PASSED. **Still NOT built (gated on PM §6 cold-start UX ruling):** the "Catching up…" loading/empty state AND the reactive refresh-after-hydrate (rows land in SQLite but a screen must re-read to show them — today that's a re-navigation/foreground). **Sequencing vs Step 9/10 is an open PM roadmap call** (§9.5).
