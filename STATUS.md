...
| **Out of beta — the log sheet, more event types, vet visits** | New (2026-09-22). Graduates `log_picker_v2`, `event_types_v2` and `vet_visits` for every account; sittings waived 2026-09-22. **The project description carries the run order** (flip → removal PR → one A-Native build → closeout) and the lanes. Step 1 shipped as #891 (CUL-961 flip, CUL-962 removal) and step 5 as #892 (CUL-503/504, one door, one confirm); CUL-905 (step 6) is next. |
