...
| **Out of beta — the log sheet, more event types, vet visits** | New (2026-09-22). Graduates `log_picker_v2`, `event_types_v2` and `vet_visits` for every account; sittings waived 2026-09-22. **The project description carries the run order** (flip → removal PR → one A-Native build → closeout) and the lanes. Step 1 shipped as #891 (CUL-961 flip, CUL-962 removal); step 5 (CUL-503/504) is in review as #892, and CUL-905 is unblocked. |
