...
- [ ] **Apply migration `016_meals_updated_at.sql` to the live DB** (Supabase SQL Editor) — **before merging the Phase 2 logic PR.** Additive/non-destructive (Migration Safety Pre-flight in the file header); row-count sanity: `SELECT count(*) FROM meals;` then `SELECT count(*) FROM meals WHERE updated_at IS NULL;` (expect 0 after).
...
- [ ] **Decide draft PR #85 (B-054 §6 cold-start UI).** On-device QA (script in PR body) → mark ready / merge, or send back. No schema. Optional call surfaced: the blocking overlay has **no max-duration timeout** (deferred per the spec's "block + await" intent; offline resolves fast so no infinite hang) — say if a defensive timeout is wanted.
