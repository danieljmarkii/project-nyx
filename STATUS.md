...
**Last updated:** 2026-06-21 — **Medication QA-pass fixes + `pm-feature-review` subagent + B-150 banner-staleness fix — shipped via #213**, reconciled onto a `main` that moved +8 (vet-report discovery #214, B-070 #211, `generate-signal` v21 #215, **B-117 PR 9 Signal medication-confounder #207**). #213 = 2 P0s (strength-confirm gate restored; account-creation null-session → "Check your email"), 4 P1s (compliance matches doses by `medication_item_id`+window; regimen spinner-hang; End-button red; meals FK sync race), the B-150 `signalTick` reactive refresh, and the product-review subagent (registered + dogfooded on medication logging → surfaced B-153 + the B-156–B-161 polish set). 18 backlog rows logged; my 6 that collided with main's new B-138/B-141–B-145 were renumbered to **B-156–B-161**. NO schema. Build phase **unchanged** — Step 10 + parallel tracks; vet-report (Step 9) discovery is the live forward track. _(Prior banner — vet-report discovery #214 — now in Current Phase + Recent Sessions.)_
