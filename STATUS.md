...
**Last updated:** 2026-06-22 — **B-156 (combo-log: medication given with food/treat) — investigation → mock → pet-owner review → gated PR-by-PR build plan (PM session).** Took the Zyrtec-in-a-Delectable idea from open question to a build-ready, gated plan. Decision record + plan: `docs/medication-food-combo-investigation.md` (§9 review, §10 PR plan); concept mock `docs/medication-food-combo-mock.html`. Key results: the recent-treats trap is **solved by construction** (the link is per-**event**, never on `food_items` — the B-010 shape); the real value is **adherence accuracy** (a pill in a not-finished vehicle is a *dose in doubt* — a false-adherence record the med n=1-never-reassures rule forbids); pet-owner review (Jordan+Sam) validated Slice C `how_given` + the trap case, flagged the combo's timing + edit model. **G1 RESOLVED (PM): the safety-prompt card auto-dismisses, with the non-negotiable `clinical-guardrails` fail-safe — an unanswered prompt records `unconfirmed`, never `given`, and resurfaces calmly.** Plan = Phase A `how_given` (A1–A3, **ungated, buildable now**) + Phase B combo (gated on **G2** edit-model + **G3** B-153/B-154 order) + Phase C engine (adversarial). Open build decisions **flagged in CLAUDE.md Open Questions** (G2/G3/G4 + Slice-B promotion). B-156 → **Next**. **NO schema, NO app code, NO build-phase change.** _Prior (concurrent, merged to main):_ **B-028 (#220): editable per-incident AI structured fields + "Edited" provenance** (adversarial PASS) — **clears the last Step 9 precondition** (B-044/B-115/B-028 all ✅); **B-147 (#223): warmed empty states**. _Still the live "where are we":_ **Vet-report (Step 9): build-ready requirements spec + 10-PR plan** — `docs/nyx-vet-report-requirements.md` (DRAFT): section contract + server-side `generate-report` architecture (deterministic, no LLM on clinical content) + 4-phase/~10-PR plan; §8.1 vet-only + §8.3 scope/cherry-pick + must-carry consumers folded in; §8.4/§8.5 ratified; **NO new schema**. **LOCKS AFTER THE REAL-VET R1/R2 PASS** — two PM gates: real-vet R1/R2 + HTML-first §8.2 ratification.
