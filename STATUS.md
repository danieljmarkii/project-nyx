...
| **Out of beta — the log sheet, more event types, vet visits** | New (2026-09-22). Graduates `log_picker_v2`, `event_types_v2` and `vet_visits` for every account; sittings waived 2026-09-22. **The project description carries the run order** (flip → removal PR → one A-Native build → closeout) and the lanes. Step 1: the flip (CUL-961) is done; the removal (CUL-962) is in review as #891, and CUL-503/504 and CUL-905 wait on it (shared files). |
