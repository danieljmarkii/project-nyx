...
**Last updated:** 2026-06-22 — **B-115 closed (#219): exact-timestamp treat re-log collapse** — new pure `collapseTreatRelogs` dedups `(foodItemId, ms)`-identical TREAT rows before `computeTopProteins`/`computeTopFoods` ranking (treat-only / exact-ms / non-null-id → the narrowest SAFE scope: never over-collapses a genuine giving, and meals are untouched so the §11 #1 meals-only finished-rate + decline lane are unaffected **by construction**; B-111 invariant + §11 #5 floor preserved; adversarial PASS; 8 tests; no schema). Clears the Step 9 **Phase-2 / PR-6 diet-confounder precondition for the EXIF-collision/future-batch case**; the rapid per-tap over-count residual (full-precision `new Date()` → distinct ms, not caught by exact-ms) → **B-163** (real-data-gated near-window call). _Prior:_ **B-044 migration-drift audit closed (#218): zero schema drift, nothing to apply** (history-backfill residual → B-162). **Vet-report (Step 9): build-ready requirements spec + 10-PR plan written** — `docs/nyx-vet-report-requirements.md` (draft). Turns the discovery (#214) + mock/review (#216) decisions into a section contract, a server-side `generate-report` architecture (pure `report.ts`/`render.ts` + I/O shell, mirrors `generate-signal`; **deterministic, no LLM on clinical content**), and a **4-phase / ~10-PR build plan**. Folds in **§8.1 vet-only (PM-decided)**, **§8.3 scope cascade** (since-visit → trial → 90d) **+ the cherry-pick guard** (custom window discloses out-of-window events), and the **must-carry consumers** as owned PRs (B-117 PR 10→PR 5 · B-040+B-102 PR 6→PR 6 · B-010→PR 7 · B-023 PR 5→PR 8 · B-143→PR 9 · B-144→PR 10). §8.4 (frequency-over-severity) + §8.5 (Established-only) ratified one-line. **NO new schema** (`vet_reports`+RLS exist; revocation reuses `token_expires_at`). **THE SPEC LOCKS AFTER THE REAL-VET R1/R2 PASS** (synthetic cold read was CLINIC-READY but can't validate trust). **Preconditions: ~~B-044~~ ✅ cleared 2026-06-22 (zero drift — `vet_visit_attachments` present, nothing to apply); B-115, B-028 remain.** NO schema, NO app code, NO build-phase change — **Step 10 + parallel tracks unchanged**. (Builds on #213 — the medication QA-pass + `pm-feature-review` subagent, just merged; see Recent Sessions.)
