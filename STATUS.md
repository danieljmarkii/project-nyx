...
| What is waiting on the PM? | **Linear — the `Needs PM` state** | one view; every issue names its single remaining step, and closing one vacates it |
...
The 2026-08-22 pass moved the state to where it is worked — 17 un-homed PM actions became Linear issues in the PM queue, the rest were matched to issues that already existed or verified as done — and left this pointer card behind. Nothing was discarded: `git log -p STATUS.md` has every word of the old file.
