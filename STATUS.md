...
- **2026-06-07 — AC-6 re-check + B-054 Phase 2 (conflict-correct LWW + B-055).** Re-ran the AC-6 logout-wipe check: corrected an earlier stale-`main`-ref error — PR #83 (Phase 1) was already merged to main 2026-06-07, so AC-6 PASSES at code level (residual = on-device gate). PM rulings: build Phase 2 now (split: correctness core this session, §6 UI follow-up); §6 = block-only-when-empty; FR-5 = server-time LWW for events + add `meals.updated_at`. Built Phase 2: migration `016_meals_updated_at.sql` (additive; nullable→backfill→NOT NULL+trigger), local schema + 3 meal-insert sites + `updateMeal*` stamp ISO `updated_at`, push sends it, `hydrateMeals`→real `'lww'` (retired `'refresh-if-synced'`), `WHERE synced=1` SQL backstop on meals/events/vet_visits. **Adversarial review BROKE the first cut** — the meals hydrate INSERT named 10 cols / 9 placeholders / 10 params with `excluded.intake_rating` on a non-inserted column, which would have thrown silently on every meal hydrate-write (the exact intake-correction safety path) — fixed, re-review PASSED. 69 tests green, tsc clean. Closed B-055; logged B-057 (guard against hydrate-SQL placeholder/param drift — the I/O shell the pure tests can't see).
