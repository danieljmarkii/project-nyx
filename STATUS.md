...
**Last updated:** 2026-06-22 — **B-028 shipped (#220, draft): editable per-incident AI structured fields + "Edited [date]" provenance** — the read-only vomit fields are now owner-editable (`VomitFieldsEditor`), with a per-field "edited" marker + one calm "Edited [date]" line; the n=1 read stays dismissible-not-editable + owner-facing-only. The **never-clobber-on-re-analysis guard** is extracted to pure fns and TESTED both halves — `buildAnalysisWriteBack` (server, deno) + `buildVomitEditWrite`/`deriveEditedFields` (client, jest); **adversarial-reviewer PASS**, code-reviewer fixes applied. **Removes the Step 9 PR-7 precondition** (the report can render owner-confirmed fields; §8.7/§11). No schema (migration 013 already carries `edited_at` + `ai_raw_payload`). _Prior session:_ **B-044 migration-drift audit closed (#218):** full live-DB reconciliation = zero schema drift. _Earlier:_ **Vet-report (Step 9): build-ready requirements spec + 10-PR plan written** — `docs/nyx-vet-report-requirements.md` (draft). Turns the discovery (#214) + mock/review (#216) decisions into a section contract, a server-side `generate-report` architecture (pure `report.ts`/`render.ts` + I/O shell, mirrors `generate-signal`; **deterministic, no LLM on clinical content**), and a **4-phase / ~10-PR build plan**. Folds in **§8.1 vet-only (PM-decided)**, **§8.3 scope cascade** (since-visit → trial → 90d) **+ the cherry-pick guard** (custom window discloses out-of-window events), and the **must-carry consumers** as owned PRs (B-117 PR 10→PR 5 · B-040+B-102 PR 6→PR 6 · B-010→PR 7 · B-023 PR 5→PR 8 · B-143→PR 9 · B-144→PR 10). §8.4 (frequency-over-severity) + §8.5 (Established-only) ratified one-line. **NO new schema** (`vet_reports`+RLS exist; revocation reuses `token_expires_at`). **THE SPEC LOCKS AFTER THE REAL-VET R1/R2 PASS** (synthetic cold read was CLINIC-READY but can't validate trust). **Preconditions: ~~B-044~~ ✅ + ~~B-028~~ ✅ both cleared 2026-06-22; B-115 remains.** NO schema, NO app code, NO build-phase change — **Step 10 + parallel tracks unchanged**. (Builds on #213 — the medication QA-pass + `pm-feature-review` subagent, just merged; see Recent Sessions.)
