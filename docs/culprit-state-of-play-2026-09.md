...
> **Read this section last, and as argument rather than fact.** It is a narrative synthesis written by the same pass that produced the lanes, it carries no evidence grades, and it states a conclusion in its first sentence. §2–§7 are the graded evidence; §8 is what broke. A reader who opens here has been handed a closing argument in the position of a fact sheet.
...
- The last session record on the submission path is **2026-08-11** (demo-account seed PR 1). Only 11 of 367 records (3.0%) are store/launch/submission-shaped **by filename**, and none since 2026-08-12. `[C]` ⚠ **This measures what got recorded, not what was chosen** — "did no submission work" and "chose building over shipping" are different claims, and the second adds a verb the data does not carry. The classification is also soft: `docs/nyx-onboarding-requirements.md` is CLAUDE.md's named spec for the **app-store-readiness** onboarding revamp, and the polish, Geist and accessibility tracks are submission-relevant by any definition Apple would recognize. Usable as a base rate; **not usable as evidence of intent.**
...
5. **Run the detectors against synthetic null records.** CUL-914 measured **the L-17 disclosure line on the Noticed pairing** true under the null at a **22–77% print rate** — ⚠ one line, one surface, one flag-gated feature shipped days ago; do not generalize it to "the engine" without saying so. Generate synthetic nulls across realistic densities and species, run the full detector suite, publish a per-detector false-positive rate. No users needed, one weekend of compute. **Score both halves:** the defect is real and shipped, *and* it exists because this project's own mandatory adversarial pass found it, filed it Urgent, and wrote it into the manual within a day — which is evidence about the operating system, not only about the bug. Read the observed yield before generating a synthetic one: production holds 5 `ai_signals` rows across 1,024 events, and what an `ai_signals` row counts (a distinct finding, a daily cache entry, a phrasing call) must be settled first, because §8 is a register of counts misread as rates.
