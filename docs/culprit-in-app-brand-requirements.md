...
2. **The register rule.** Night grounds appear where the app is **working on the pet's behalf** — the Landing, loading beyond a beat, the night moment, **and the Daily Recap (`/day-summary`), the evening read-back the 9pm notification opens** *(amendment 2026-08-15 — D-3 of the notifications-v2 track, PM-ruled "the evening ritual/night register"; wording written under the PM's same-day take-point delegation)*. The recap qualifies by the rule's own logic: it is the app's work **presented back** — the app kept the day so the owner didn't have to — it is presentation-only (doorway rows, no edit affordances), and its primary arrival is literally at night. It renders **always-night** (one identity, whether opened at 9pm from the notification or midday from Home). Darkness admits no new register: symptom rows ride `colorEventSymptomOnNight`, the med slate gains a night sibling token (minted with the build), and no verdict or reassurance language gains admission by ground. Capture and *working* record surfaces (quick-log, History, Foods, Profile, the vet report) stay the shipped light system, untouched. The Signal card's ground is governed by §7.
3. **One interactive accent.** Teal `colorAccent #00C2A8` remains the sole tappable/live accent on every ground. Indigo is only ever a world/ground color. Red keeps its shipped meanings (symptom / destructive) and is never decorative. *(Amendment 2026-09-26, CUL-322, D3 = C, PM-ruled: **one exception, the FAB.** Its disc is `colorBrandNightElevated #251F57` carrying the teal `colorAccent` plus, 14.3:1 on the app ground and 6.57:1 plus on disc. The teal is still the live glyph and the indigo is still its ground, so the rule's protection holds for as long as no other control is indigo; widening indigo to the app's action colour is parked as CUL-1279.)*
