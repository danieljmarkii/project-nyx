# Culprit Home Screen Widget — Requirements (v1.0 — FROZEN)

> 🧊 **FROZEN 2026-08-02 — the as-built record of the v1 capture widget (TestFlight build 35, PRs W1–W5).** Superseded as the living spec by **`docs/nyx-widget-requirements.md` v2.0** (the round-4–7 informational redesign, PM-ratified R7-1). Kept verbatim below because this document describes a binary that is still installed and a design (`widgets/CulpritWidget.tsx` as of #428) that is still on `main` until the v2 rebuild lands. Do not edit; do not build against it.
