...
- **The spec is v1.3** (`docs/nyx-history-v2-requirements.md`; HV-1's #907 took v1.2 for its own ruling, CUL-1183, so the merge of `main` renumbered this branch's edits): §0.5 records both rulings; §3.2 carries the two count-line clauses; §3.9's trial row and §5.2 say the window follows `isTrialRunning` (§11), which stood unobjected on CUL-1189; §7 gains AC 40 and 41. The words are HV-7's to build (CUL-1164 has them).
