...
## Addendum — the drawn sheets, read cold (same day, after publishing)

The PM asked whether the round is a meaningful improvement. The interviews above were taken *before* anything was drawn, so three isolated reads were run on the drawn lead sheets (direction C, dog and cat) against today's page 1: the `vet-report-cold-read` gate (Dr. Chen cold), the skeptical GP, and Sam.

**Verdict, in their words.** Dr. Chen: NOT READY on both sheets; "more than a tidier page … but as drawn it is not yet an improvement, and on the dog it is a regression"; "faster to a wrong answer is worse than slower to a right one." Skeptical GP: "a meaningful improvement in shape, carrying five new honesty leaks, each a one-line fix"; the thing that flips it back to a tidier page is the before/from split as drawn ("the dumbbell in words"). Sam: "meaningfully better; today's page she asked what app is this, this one she asks when did it switch to bile"; insists the flag line stop saying "Owner-reviewed" beside "unconfirmed".

**What all three credit.** The single axis; the "May 14 · logging began (nothing before is a gap, not a clean week)" marker; the blind-spots line under the flags; the per-incident photo reads in date order (the food → bile shift, "the strongest clinical addition in the whole redesign"); the unassessed segment drawn outside the proportion bar.

**The faults, sorted by who owns them.**
- *Mock errors, one line each:* the chart gutter's "before May 12: 5 · from: 6" with no denominators (11 d vs 52 d; a §5.1 breach that inverts the trend at second 25); the "engine change" build annotation printed on the sheet; "Owner-reviewed" beside "AI read · unconfirmed" (§5.10 says owner-reviewable unless edited); "Intake not directly observed" four times on one sheet; the "Off-diet feedings" label asserting a check the page says never happened; human food folded into the treats row with no count (§4 B-102); one hatch texture carrying three meanings; ten feedings drawn as nine marks (same-day merge); the weekly bins straddling the May 12 line so the picture sums to 6 where the text says 5.
- *Compression that deleted a non-detection note, the n=1 rule at page level:* the vomit non-clearance line on a record with no photos ("0 of 11 have a legible read" today); the stool blood-and-mucus limitation on a dog with seven loose stools; the medication day-spread ("on 14 of 14 days", §4 B-117); the per-half logging-effort control ("24 of 26 … 23 of 26", the proof the decline is not owner fatigue); the bounded allowed-list gap ("for part of this window"); the per-protein weekly exposure (on a hydrolysed trial the protein is the question); the estimated/window timestamp count (§3.9, computed at `report.ts:3717`, never drawn).
- *Fixture and engine artifacts the sheet printed as findings:* "23 of 23 episodes fell between 10:00 and 14:00" (every fixture event is stamped at the same hour; the report does not render a time-of-day lane; no witnessed/found split, B-010); "free-fed since Mar 1" (the renderer returns "ongoing, start not recorded" for exactly this, `render.ts:4125`).

**What round 2 must carry (Dr. Chen's list, verbatim in spirit):** the proposal's axis, blind-spots line, logging-began marker, date-ordered photo reads and dashed unassessed segment, with today's equal-half trend arithmetic (or the intervention split *with* both denominators in the gutter), today's logging-effort control, today's two non-detection notes, today's per-protein breakdown, today's bounded allowed-list disclosure, and both fixture artifacts deleted. The honest word budget for that page is nearer 600 than 490.

**Lesson worth carrying.** The compression fell on exactly the blocks whose job is to prevent absence reading as wellness. "Say once what the chart cannot carry" means *once*, not *never*; the mock read it as never. A caveat that protects a number belongs in the chart; a caveat about what the record cannot see belongs on the page, once, and its deletion is the same failure the n=1 rule guards at the incident level.
