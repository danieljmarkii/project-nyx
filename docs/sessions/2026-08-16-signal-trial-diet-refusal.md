...
- **`app/(tabs)/index.tsx`** — Home computes `suppressTrialResponse` from the `trialInput` it already loads (no second read), **failing closed** on stale/unloaded facts: `trialFactsFresh && trialInput ? isAnimalNotEating(trialInput) : !trialFactsFresh`. Passed to `SignalZone` beside `trialRunning`. **Also** withholds the trial STRIP's standing vomit line under the same fail-closed rule (round-2 residual): `resolveTrialStrip` reads the retained `trialInput` directly, so across a pet switch it could lag the previous (eating) pet's reassuring count over a now-active refuser — nulling `trialResponseLine` until the active pet's facts are confirmed keeps the strip and the card from ever disagreeing about the same refusal, even mid-switch (steady state byte-identical).
...
- **`adversarial-reviewer` (DoD-mandatory) — round 1 FAIL → fixed; round 2 <TBD>.**
  ...
  - **Round 2 (re-review of the fixes) — PASS.** All three prior findings held under a concrete counterexample: fail-closed across every window (cold start / switch both directions / load error / hydration reload), batching-independent, no same-pet flicker; only `fewer` dropped, `more` escalation kept (`comparisonDirection` always present on any cache that can reach the filter); the empty-stack residual is safe-direction silence (CUL-527). It surfaced **one new residual** — the Home strip's CUL-13 vomit line read `trialInput` directly and could lag a stale reassuring count across a switch (pre-existing, dormant, LOW). **Closed here** (the strip fix above), on the reviewer's own recommendation, since it is the same reassurance-over-refusal hazard and made the card/strip consistency thesis literally true. `isAnimalNotEating` proven identical to the strip's old inline predicate; the additive `inputIsForActivePet` breaks no other `useDietTrial` consumer.
...
- [x] **Adversarial review** — round 1 FAIL (3 findings) → fixed; round 2 **PASS** on the fixes; the one new residual (strip vomit-line staleness) closed in-session.
- **Persona sign-off:** Data/Adversarial ✓ (two falsification rounds; round-2 PASS, every load/switch/error window fails closed) · Dr. Chen ✓ (the never-reassure asymmetry: the card is withheld, never a reassuring claim minted, and now under load + mid-switch too; escalations are kept) · Designer ✓ (no visual change; the card simply doesn't render) · Engineer ✓ (one predicate, no second read, no bundle; fail-closed freshness) · QA ✓ (237 suites / 5276 green, tsc clean, CI green).
