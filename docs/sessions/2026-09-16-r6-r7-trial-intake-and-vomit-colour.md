...
Session S2 of the v15 cold-read re-cut run order. Two findings were built; **one shipped and one
did not**.

- **R-7 (CUL-981) — shipped via #863.** The vomit colour tally. Survived every attack in all three
  adversarial passes, and was split onto its own branch off `main` once R-6 was withheld.
- **R-6 (CUL-980) — withheld.** Built, reviewed three times, **FAIL** every time, and the third
  pass established that the second round of fixes had introduced a regression against the state
  before the PR started. #860 is retitled `[DO NOT MERGE]` and stays open with the root cause
  written up; CUL-980 is back on the board unbuilt.

Everything below describes what was built. The section **"The re-run, which is the part that
matters"** and **"The third pass, and the decision to split"** are the ones worth reading.
...
## The third pass, and the decision to split

The re-run's own fixes were re-reviewed, because C-19 does not stop after one round. That third
pass returned **FAIL** with eight defects, **six of them net-new in the previous fix commit**, and
one of those was a regression against the pre-PR tree:

> The merge-by-label added to fix the duplicate-row finding **pools two genuinely different
> formats** whenever their labels collide — and they collide on the modal setup. The label is
> `Brand Product (Form)` and `lib/foodFormat.ts` emits no form word for `other`, null, or an
> unlearned enum value; a prescription line stocked wet + dry has one brand and one product name.
> Executed side by side against `86f543a`: **before** the PR, `— 12 rated meals, 0 fully eaten
> (ate most ×11 · refused ×1)`; **after** the fixes, one pooled row and a `23 / 35` headline. The
> wet half's 0-of-12 — the founding finding of CUL-980 — was on the page before and gone after.

**The root cause, which is the only durable thing this issue produced.** `TrialPermittedFood` drops
`foodItemId` at the type boundary, and the renderer only ever sees `role === 'primary_diet'` while
`lib/dietTrialSetup.ts` writes every owner-added food as `permitted_treat`/`permitted_other` and
the mid-trial-add path **can never write `primary_diet`**. So the block had been making
food-identity and list-membership claims with no data to support either — which is why each
patched sentence kept turning out falsifiable, including the partition sentence's inclusion rule,
false on the routine setup and contradicting the exposure tile on the same row.

**The decision.** Three consecutive fix rounds each introduced new reassuring-direction defects
into a clinical document. That is a pattern, not three accidents, and a fourth round from the same
session was the wrong bet however tractable each individual finding looked. So: R-7 ships, R-6 goes
back. The PM was offered the choice and deferred; the split is the conservative half of it — R-7
had survived everything and shares no data or logic with R-6, so holding it would have been pure
loss.

R-6's rework, in the order it should happen: carry `foodItemId` and the real role set through to
the renderer; re-derive the block's claims from what that data supports rather than repairing
sentences one at a time; and delete the tests that encode the unsupportable assumptions — the
duplicate-merge test and the n=1 cell test each stop one assertion short of the defect they cover.

## Deploy

`generate-report` re-fingerprinted for **R-7 only**, still **`pending`** on the same owed Codespace
deploy as R-1, R-2, R-5, R-11, R-16, R-17 and R-18 — one deploy serves all of them, and it cannot
run from an agent session (`SUPABASE_ACCESS_TOKEN` is a Codespace secret; the bundle is far past
the MCP inline path). **Merging did not deploy the function.** The ledger entry carries the
after-deploy checks and records why R-6 is not in it.
