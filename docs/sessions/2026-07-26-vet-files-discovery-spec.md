# Vet Files (B-478) — discovery → build-ready spec + design-locked mocks, in one session
...
- Backlog: **B-478** (the feature; renumbered from B-467 — see below), **B-479** (search, deferred out of v1), **B-480** (report paperclip, deferred; explicitly NOT D8-gated).
...
4. **Mock round 2** (demo pet switched to Sam's cats Pixel/Juniper so multi-pet gaps stay visible) → PM ruled **D11** chips out (Jordan's position; list-row Name affordance is the recovery), **D12** search out (→ B-479), **D13** multi-pet via **duplicate-on-add** (independent copy per pet — a shared-document model breaks the path CHECK, bucket policies, and deletion cascade; schema stays `pet_id NOT NULL`), **D14** report paperclip out (→ B-480).
...
- **D14's clause worth remembering:** the paperclip (attach a stored PDF to a report send) is *not* an AI read — when B-480 is picked up it needs a scope ruling only. Recorded explicitly so a future session doesn't treat D8 as blocking it.

## What broke / friction

- **B-435 ID race — TWICE in one session.** First a sibling claimed B-467 on `main` (protein-rankings) while ours was unmerged → renumbered to B-477; then another sibling claimed B-477 on `main` (the provider-string leak class, #470) → renumbered again to **B-478** (search → B-479, paperclip → B-480), each time across five files. The evidence for B-435's fix is getting expensive; whoever picks B-435 up should read this session.
