...
- **CUL-571** (High) — `lib/metricDetail.ts`'s Week tab still states an ungated week-over-week symptom direction, two taps from the card just fixed. The re-pass sharpened this: `lib/analytics.ts` documents raw counts as a *deliberate* choice ("must match the History timeline"), so the unit contradiction was **moved** from a same-screen adjacency to a one-tap one, not removed — and two comments in the repo now assert opposite units with no recorded decision between them.
- **CUL-581** (Medium) — `photoComposition.collapseComposition` is a third body of the collapse, exempted by name in the guard rather than re-based in this PR.

## The adversarial re-pass — a second FAIL, and one real regression

The code changed substantially after the first reviews, so the earlier pass no longer covered what ships and a re-pass was run rather than signing the DoD line off on superseded code. It returned **FAIL** again. The headline safety fix survived every attack; four other things did not.

**1 — The tie-break preferred the resolving symptom. A regression against `main`, in the improvement-leaning direction this whole PR exists to remove.** `summarizeSymptomTrend` imported `detectReflections`' "larger fall" tie-break without importing its candidate filter (`currentCount <= priorCount`). Inside that filter the rule only ever orders symptoms that are *already falling*; lifted out of it, it systematically prefers the symptom going **away** over the one that just **arrived**.

Reproduced and re-run locally: a cat with **four vomits inside ninety minutes yesterday** beside a resolving itch produced `{dominantSymptomType: 'itch', thisWeekSymptomCount: 1}` — a card headed **"Itch/Scratch"** over a chart whose tallest column was entirely vomit, with the vomiting named nowhere. A steady chronic vomit (3 this week / 3 last) lost to a falling itch (3 / 5) the same way. **Fixed** by inverting the comparison to prefer the larger *rise*; both cases now name the vomiting, and all three reproductions are pinned as tests. (One of my own tests had encoded the buggy behaviour as expected — it asserted the falling itch should win — and was rewritten.)

**2 — The guard did not guard.** Demonstrated three ways: two renames (`prev`→`last`, `gapMs`→`threshold`) defeated it inside the scanned tree; a second collapse written *inside `detection.ts`* passed; and the whole `supabase/functions/` tree was unscanned — where `photoComposition.collapseComposition` had been sitting as a live third body all along, reading a **second independent hardcoded 3** (`DEFAULT_MEAL_TIMING_CONFIG.episodeGapHours`). **Fixed:** the scan now covers `supabase/functions/`, the shape match covers renamed cursors and thresholds, the rename-defeat case is pinned as a self-test, and `SYMPTOM_EPISODE_GAP_HOURS` now *resolves to* `DEFAULT_MEAL_TIMING_CONFIG.episodeGapHours` so the two constants cannot diverge. `collapseComposition` itself is a **named exemption** with its reasoning inline (→ CUL-581), and the guard now **states its own limit**: a syntactic tripwire for the accidental second copy — which is how all three real ones arrived — not a proof of uniqueness.

**3 — A second DST bug, three lines below the one already fixed.** `buildBuckets` derived its 14 day-keys with calendar `setDate` and then `toISOString()`, mixing the two bases the fetch-bound fix had just unmixed. Swept across DST-observing zones it yields **13 distinct keys for 14 columns ~13 times a year**: one UTC day's events land in no bucket, a column is permanently empty, and `daysWithAnyEvent` drops by one — which on a marginal pet flips the card to the "a few more days of logs" empty state. The `find`→`Map` swap earlier in this session also changed *which* duplicate won. **Fixed** by stepping the keys in fixed epoch days. This does not resolve the deeper B-421 local-vs-UTC question (already named in this file by `lib/widgetSnapshotV2.ts`); it makes the 14 keys distinct and contiguous.

**4 — Deliberately not fixed.** `hasEnoughData >= 3` still hides the chart for six vomits across only two days with no meals logged — that is `main`'s behaviour, so the bars revert restored parity rather than closing the hazard, and it is worth saying plainly rather than letting the fix note imply otherwise.

**What the re-pass confirmed held:** no branch in `SymptomChart` can emit a direction word, arrow, accent, or prior-window number; the zero-count suppression never becomes a false claim; the delegation is behaviour-identical *by argument* (a gap break can only occur at the first element of a run of equal instants, so the cursor walk selects exactly what the old loop did) rather than only by fuzz; and the differential test has real power — mutation-testing it caught a last-at-onset mutant in 10,993/40,000 trials, an unchained-gap mutant in 6,197, and a `>=` boundary mutant in 3,919.
