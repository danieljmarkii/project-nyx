...
Filed as **B-494** with four options. Worth noting that "accept it" is defensible — the documented deploy path is the MCP, not `db push` — but it should be a decision rather than an accident, and if chosen, the runbook should warn against `db push` explicitly.

## Scope

Live tracking-table write + docs only. No app code, no schema change, no Edge Function deploy. `B-142` merged into `B-162` as the duplicate row it was; both opened independently off the same B-044 finding a day apart.
