...
- **`adversarial-reviewer` (MANDATORY per CUL-8) + `code-reviewer`:** both dispatched at wrap; **results land as a follow-up commit on this draft PR** (the concrete counterexamples tried against each invariant — never-verdict / fail-toward-escalation density gate / the §8.5 trigger's null calibration / one-predicate day math / honest counts — and their verdicts, plus any fixes). The draft stays NOT-ready-to-merge until the mandatory adversarial pass is recorded green.
