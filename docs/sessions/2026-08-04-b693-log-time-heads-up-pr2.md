...
3. **Should an intake-chip tap protect the flagged card's window** rather than collapse it to B-014's 1.5s confirm hold? (B-703 #1.)

## PM action items

None to execute. The three items above are **decisions**, routed to the PM/Dr. Chen (B-702 / B-703), not actions. Surface-only PR, CI green, no secret/migration/deploy.
