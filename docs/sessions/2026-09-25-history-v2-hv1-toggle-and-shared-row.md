...
- Merge order with HV-2 and HV-5: whichever lands after this PR applies its fixture patch to `lib/dayNodes.test.ts` (posted on CUL-1159 and CUL-1158) and re-runs CI after updating from main, because a check that went green before the other merged cannot show the break.
