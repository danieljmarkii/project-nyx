...
**The adversarial pass, round two (on the ratio form, driven end-to-end through `assembleReport → renderReport`):** **FAIL**, three breaks, and the reviewer's own verdict on the shape was that *always-render is safer and I'd keep it even given B1* — a suppression predicate is a cliff that has to be right at exactly one point, and round one had broken it on both sides of that point; B1 is a denominator choice inside the always-render shape, not an argument for going back.

1. **B1 — the ratio's denominator was derived from its own numerator's endpoints**, so a contiguous course printed *14 of the 14 days* whatever sat outside it: CUL-976's own reference record (28 doses Jul 17–30, End tapped Aug 9) put three 14s in a row in front of a ten-day tail hole; a six-week tail and a sixteen-day head hole printed identically. Fixed here: a contiguous course states the positive fact — *The 28 administered doses fell on all 14 days, Jul 17 – Jul 30, 2026 (28 prescribed doses at 2×/day take 14 days)* — and only a holed course carries a ratio (*fell on 10 of the 29 days from Jul 1 to Jul 29, 2026*); the head or tail hole is read off the regimen clause beside it, which now carries its year.
2. **B2 — record-scoped dates rendered year-less**: a year-long course printed *305 of the 365 days from Jun 21 to Jun 20*, a range that runs backwards, and a 2025 course's dosing read as inside this year's window one clause before the page said no doses were logged in it (C-19, C-37). Fixed here: `fmtRange` / `fmtSpanFromTo` / `fmtDayYear` stamp the year once per pair on the span, on the regimen's dates, and on an active course's *since*.
3. **B3 — the neighbouring window clause can print *20 of 3 days of the course***, because `lib/medications.ts`'s explicit-link attribution pass is unbounded by the regimen's span. Not this pass's region: recorded on **CUL-992** (a dose before the recorded start is the mirror of one after its end).

The near-breaks, each fixed here: *Those doses* had the prescribed set as its nearest antecedent, so the subject now names its population (*The 28 administered doses*) and the parenthetical names its (*28 prescribed doses at 2×/day*, or *40 doses at 2×/day take 20 days; 28 were prescribed* when delivery exceeded the plan); `dosingDaysFor` computed in IEEE doubles and understated the need by a day on 367 two-decimal paces, always in the reassuring direction (7 / 0.14 = 49.999…), so it now computes in hundredths, the precision of `doses_per_day`'s `NUMERIC(4,2)`; the recorded end is no longer restated at the sentence's tail. Stated blind spots, in the docstring: an under-logged course reads as a sparse dosing pattern; a day count cannot say where the holes are (Appendix D lists the dates — CUL-1013). Held on re-attack: n=1 singular agreement, 28 doses on one day, over-delivery on more days than the need, every timezone edge (one `localDayKey`, no C-40 bound), `not_tracked` double-guarded, supplements excluded. Twenty-eight mutations against the final form, all caught — the third survivor of the first run was a real gap, an active course's year-stamped *since*, and gained its assertion.

**DoD line (Biostatistician):** re-attacked the reworked dosing sentence four times in all. The dispatched suppression predicate: one post-End dose deleted the sentence over a fifteen-day hole, a first–last range hid a nineteen-day interior hole, a one-dose shortfall re-armed it as a ten-day abandonment ✗. The ratio form: *14 of the 14* in front of a ten-day tail hole; *from Jun 21 to Jun 20* on a year-long course ✗. The final form: contiguous → *all 14 days* with the tail hole read off the year-stamped regimen clause; 305 of 365 from Jun 21, 2025 to Jun 20, 2026; 0.14×/day → 51 days, not 50; the equal-delivery course never reads as over-delivery ✓.

## Persona sign-off

Designer ✓ (A.1–A.6, the legend, the voice pass: no `!`, no owner address on the vet page, plain words) — Engineer ✓ (the tail container, the ledger, the guards by mutation) — Data Scientist ✓ (`timeConfidence` one predicate; the dosing sentence's arithmetic) — Dr. Chen ✓ via `vet-report-cold-read`, two rounds — Biostatistician via `adversarial-reviewer`, two rounds, both FAIL and both fixed here (the line is in *Round two*) — Trust & Safety N/A (no access path changed) — QA ✓ (the DoD lines in the PR).
