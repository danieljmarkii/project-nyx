...
- Persona sign-off: Data Scientist / Dr. Chen (honesty properties held under executed attack) · Engineer (schema-correct, upsert-only, glob placement) · Trust & Safety (assertion prelude + food-ownership self-check + no fabricated AI read). `code-reviewer` — _outcome in the PR._ No `nyx-voice` (no owner-facing copy).
