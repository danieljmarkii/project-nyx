...
- **B-700** — the chronicity safety flag renders a lookback-bounded `firstOnsetIso` as an absolute "first logged" date that can contradict Appendix A and **under-state duration** (wrong direction for a safety lane). **Filed `Now`, flagged as gating the B-494 `generate-report` redeploy** (the redeploy ships the contradiction otherwise). Clinical → its own `adversarial-reviewer` gate.
- **B-701** — an OTC antihistamine reads as "a supplement" in the Reading-the-trend line (the pre-existing `is_prescription=false ⇒ "supplement"` mapping), while it's correctly a medication in the new table + Appendix D.

## Deploy

**No standalone deploy** — per D2/spec, this rides the **B-494 `generate-report` redeploy**, which is separately gated (B-494's own adversarial + Dr. Chen sign-off, plus now **B-700**, the chronicity-onset fix this session surfaced). Until that redeploy, the live report keeps the pre-PR-7 behaviour; the lifetime table is code-complete and test-covered, waiting on the shared redeploy vehicle.

## Track status

**The medication-history track (B-140) is complete** — PRs 1–5 all shipped (#585/#587/#588/#589/#591). PR 4 (the rundown "past 12 months" block, D3 ratified) landed in parallel via #589; this PR (#591) is the last, the *report* tense. Invariants H1–H4 hold across all five surfaces; B-394 owns the *forward* tense next. Two pre-existing report defects this session surfaced (B-700 chronicity-onset, B-701 OTC-labeling) are filed, with B-700 gating the B-494 redeploy.
