...
**Operator:** [YOUR FULL LEGAL NAME], an individual
...
- When you photograph a food package, the app reads the photo's "date taken" metadata (EXIF) to record when the photo was taken. We do not collect your device's location, and we do not use photo location metadata: every photo is re-encoded as it is uploaded, which strips location and other embedded metadata before it leaves your device (see Section 3).

**Usage metering (generated by the app):**
- To keep AI features working for everyone, we count how many times your account uses each AI feature — food-label extraction, medication-label extraction, symptom-photo reads, and pattern phrasing — per day. These counts enforce fair-use limits so no single account can exhaust the service. They are internal operational data, tied to your account, never shared, and never used for any other purpose.
...
Vet report generation does not use AI at all — reports are assembled deterministically from your logs. Photos are already stripped of location and other metadata when you upload them (Section 1), and any photo embedded into a report is re-encoded again server-side.
...
Culprit is offered in the United States, and your data is processed in the United States.
...
- **Export and anything else:** contact us at support@getculprit.app and we will respond within the timeframe required by applicable law. (A self-serve export is planned.)
...
Questions, requests, or complaints: support@getculprit.app. You may also have the right to lodge a complaint with your local data protection authority.
