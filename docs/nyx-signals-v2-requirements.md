...
**Version:** 1.1 — **PM-ratified 2026-08-14** ("This reads right to me"); **v1.1 (2026-08-14): §8 Q2 `longGapHours` ruled → 6h (Dr. Chen, CUL-16 / PR #638)** | **Date:** 2026-08-14
...
**Sibling tracks:** B-721 Signal/Home design uplift (this track composes with it, supersedes nothing); **B-753 post-prandial receipt (Option A, #636 — landed 2026-08-14 in parallel with this spec's ratification):** the ⑤ dot lane now plots **real per-episode times** (expanded-early scale, `eligibleMinutes[]` + `timingReliable` payload, fail-safe split when reliability is unknown; its B-754 gate-predicate work is upstream of any confident lane) — **this track's timing surfaces inherit that geometry**: the A2 card's rapid lane and the Patterns Timing panel adopt Option-A semantics (real positions, honest precision, the reliability gate), never the old even-spread; CUL-12/CUL-18 carry the note; mock round 2 re-renders on Option-A geometry. B-756 event-type taxonomy (its own doc — the cough lane here is gated on its cough type); B-494 (gates `generate-report` redeploys — the report-side additions here queue behind that chain, never their own).
...
| D10 | `longGapHours` phenotype boundary (§8 Q2) | **6h.** The empty-stomach band boundary — L1's numerator, L3's retained-food join, the A2 card's third band. Anchored to feline solid-phase gastric emptying (t½ median ~5.5h, range 3.5–12.8h; 75% emptied ~4.8h; some cats delayed >5h at baseline), **not** Nyx's record — at 4h a solid meal is still ≳half in the stomach so the empty-stomach label is not defensible; 6h is past half-emptying for nearly all cats. Specificity over sensitivity for a band whose label asserts physiology; the property sweep locks the floors *at* 6h and never moves the boundary (G6). | Dr. Chen; PM deferred 2026-08-14 (CUL-16 / PR #638) |
...
- Fires on the fraction of eligible episodes **≥ `longGapHours`** since the last feeding. `longGapHours` = **6h** (ruled 2026-08-14 — owned Dr. Chen calibration, PM deferred; CUL-16 / PR #638; feline solid-phase gastric-emptying literature is the anchor, not Nyx's record — G6; at 4h a solid meal is still ≳half in the stomach so the empty-stomach label is not defensible, while 6h is past half-emptying for nearly all cats and clears the slow-motility baseline). Floors: `minLongGapEpisodes` 3, `minEligibleEpisodes` 6 (shared with ⑤), `minLongGapFraction` 0.25, all provisional until the property sweep.
...
**4.1 The timing card (A2 + A3's expand).** Face: lead sentence carrying both phenotypes count-anchored; Shape-C three-row compare (≤30 min / in between 30 min–6h / 6h+, both counts printed); meta row (badge, sample line: "N timed of M episodes · 60 days"). Expand: per-phenotype dot lanes (Shape A, the `ate · 30m · 2h · 6h+` axis), the clock lane for the early-am cluster, the control side ("mornings with a meal and no episode: N of M"; "episodes not near any logged meal: K — we can't time those"), the §5.4 med line where applicable, L3's composition lines, and the for-your-vet register (descriptors, never labels).
...
2. **`longGapHours`** 4 vs 6 — **RESOLVED 2026-08-14 → 6h** (Dr. Chen; PM deferred. Feline solid-phase gastric-emptying anchor. See CUL-16 / PR #638; brief `docs/nyx-signals-v2-longgaphours-decision.md`).
