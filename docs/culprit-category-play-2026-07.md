...
- Security triad: **B-397** (delete `view-report` + `zz-deploy-probe`), **B-431** (anon INSERT on `nyx-pet-photos`), **B-248** (owner-scope `nyx-vet-attachments`) — **all three closed 2026-07-26 (#465 / #460 / #466) while this plan was in review.** Gate 0's security half is done; what remains here is B-280 PRs 2–4 and the honesty fixes below.
...
**B-417 PRs 4–7** (trial card v2, off-diet exposure detection, completion milestone, vet-report render) **+ B-351 Phase B** (set-membership correlation in the engine) **+ B-416** (re-derive `proteins` for the live single-protein rows). *Moving fast while this plan was in review — same-day landings 2026-07-26: B-417 PR 4 (#454) and PR 5 (#459, the one shared off-diet predicate), B-416 (#452), and B-351 Phase B/slice 6 (#458 — the set-membership engine). Remaining for this swing: B-417 PRs 6–7 + the slice-6 follow-ups (B-467–B-470).* Done means: an owner starts a trial in-app, the engine models *every* protein in every food (a rabbit-and-chicken food never again reads as clean rabbit), and the report answers its own first question — "is this diet trial working?" — with real rows. This is the claim no competitor can currently make at any price: *the only product that models what is actually in the food.*
