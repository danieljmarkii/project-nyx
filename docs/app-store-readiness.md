...
| **B-267** | iOS camera + photo-library **usage strings** missing from `app.json` | **Open — newly surfaced** | Dir. of Engineering | Add `expo-image-picker` config plugin + explicit `NSCameraUsageDescription` / `NSPhotoLibraryUsageDescription` / `NSMicrophoneUsageDescription` copy (the plugin sets all three; camera capture can include video). Strings must state the *purpose* in Nyx's voice — Apple rejects generic "This app needs camera access" copy. Verify the built Info.plist carries all keys, **and** (same built-artifact check) that the aggregated `PrivacyInfo.xcprivacy` privacy manifest is present — Expo SDK 54 auto-aggregates required-reason declarations; verify, don't assume. iOS **crashes** without the usage strings. |
