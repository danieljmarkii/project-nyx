...
_This is the synthesis the PM asked for: a single "what stands between us and the App Store" view, assembled from Linear (team Culprit — the backlog, migrated from `docs/backlog.md` 2026-08-15), `STATUS.md`, and a direct read of the repo (`app.json`, `package.json`, signup/auth copy). It realizes the checklist that **B-002** anticipated ("flesh out as prod timing comes into view"). It is a reconciliation of existing tracked work plus four newly-surfaced items (B-267–B-270) — **no new product scope is invented**; freemium/paywall product decisions remain Open Questions for the PM._

**How to read it:** items are grouped by *what they gate*, not by priority. Tier 1 = the app is rejected or crashes without it. Tier 2 = the submission form can't be completed without it. Tier 3 = a PM decision unblocks a chunk of Tier-1/2 work. Tier 4/5 = strongly-recommended-but-not-a-hard-gate. Each row keys to its tracking issue (a legacy `B-NNN` now maps to the migrated Linear issue, team Culprit) so this doc stays a lens, not a second source of truth.
...
| **B-269** | Listing assets: screenshots, marketing icon, description, keywords, age rating | **Open — newly surfaced** | Produce all listing metadata. Screenshot requirement is one **6.9-inch iPhone set** (smaller sizes scale down automatically); iPad 13-inch not required — **`supportsTablet: false` shipped 2026-07-24** (guide step 5; zero iPad QA has been run, iPad screenshots are extra work, and a phone-stretched layout on a 13" iPad is a quality-bar risk; add iPad when it's designed, not defaulted). Takes effect on the next native build-cut, not OTA. Age rating: Apple's **2025 revised questionnaire** (4+/9+/13+/16+/18+ tiers) — answer the medical/treatment-information question thoughtfully; B-270's disclaimer framing helps. |
...
| ~~**B-248**~~ ✅ / **B-249 / B-255** | ~~`nyx-vet-attachments` bucket-wide read~~ + retention hardening (orphaned event-photo objects; expired vet-report reaping) | **B-248 done 2026-07-26** (migration 043, PR #466, applied) — the bucket is owner-scoped and `vet_visit_attachments.storage_path` is prefix-bound (B-466 folded in); this was the one item here that *was* a live cross-tenant read, dormant only at n=1. B-249/B-255 remain: T&S / data-minimization retention hygiene before scale, not live leaks. |
...
- **Product Owner / Backlog Steward** — reconciled B-039 (draft→merged head); added B-267–B-270 as proactive tracking rows (sanctioned, not scope-invention). _(A 2026-07-07 note here about migrating rows up within `docs/backlog.md` is now moot — the backlog migrated to Linear on 2026-08-15.)_
