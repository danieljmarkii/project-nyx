...
**Version:** 1.0 — build-ready for **Phase 1** | **Last Updated:** 2026-08-06
**Backlog:** B-712 (program), B-713 (pre-mortem guardrails)
**Status:** PM **phased-GO** 2026-08-06. Round-1 mocks shipped via #601 → `docs/culprit-beta-features-mockups.html` (artifact 🧪). Session record: `docs/sessions/2026-08-06-beta-features-program-mocks.md`.
...
| **OPEN-1** | **Name:** "Beta features" / "Labs" / "Early access". | Open — mock uses "Beta features". "Early access" reads best if D1 flips to Premium later. Decide before PR 3 copy. |
| **OPEN-2** | The optional **"N on"** count on the Settings row. | Open — cosmetic; decide at PR 3. |
| **OPEN-3** | **Feedback channel:** reuse Settings → Support "Share feedback", or a beta-specific prompt. | Open — reuse is the PR-3 default; a beta-specific channel is a B-713 #4 (measurement) call. |
...
### §4.3 Deferred to the Phase-2 scoping pass (do not build in Phase 1)
- **Graduation / kill policy** + per-beta owner & expiry (the graveyard, B-713 #3).
- **Measurement + consent** plan (B-713 #4; the widget is near-unobservable — iOS won't say if it's on a home screen; ties to B-016/B-047 analytics/T&S).
- **Scale mechanism** — the hand-edited allowlist → premium check or a `beta_members` table + join flow (B-713 #5).
- **Premium-graduation "taketh away"** — grandfather testers; never paywall a care feature (B-713 #6, Pets > $).
...
- **Opt-in store (D4):** a small local pref (`lib/betaFeatures.ts` — an AsyncStorage-backed Zustand store, or extend an existing prefs store), default off, per feature key. **Add its AsyncStorage key to `wipeLocalSession`** (CLAUDE.md wipe rule — account state outside SQLite must be wiped).
...
- **PR 3:** "Ship Beta Features PR 3 — `app/settings/beta.tsx` (copy of notifications.tsx) + the local opt-in store (default off, wiped in `wipeLocalSession`) + the Preferences row (eligible-gated) + route registration; rewire `useWidgetSnapshots` to `eligible && optedIn`. First read `docs/nyx-beta-features-requirements.md` §5 PR 3 and the §4.3 guardrail-scoping outcome."
...
1. **Name** — Beta features / Labs / Early access (OPEN-1; before PR 3/4 copy).
2. **"N on" count** on the Settings row (OPEN-2; PR 3).
3. **Feedback channel** — reuse Support row vs. beta-specific (OPEN-3; PR 3/4, ties to B-713 #4).
4. **Demo-account handling** for App Review (PR 2; recommend: neutral empty door, don't allowlist the review account).
5. **§4.3 scoping pass** — graduation policy, measurement/consent, scale mechanism — must land before Phase 2 (PR 3).
