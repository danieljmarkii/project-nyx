...
**Option C — chips grouped by family.** The observed-absence chip and the intake router first, then species-keyed words in fixed family order (§4), each a single tap; one word saves on hold or `Done`; multi-select allowed because a morning can hold two signs (hiding *and* not grooming); "Something else ›" opens the full grid. What it keeps: the affect-labelling move (forced choice among near neighbours) applied to observation; every word in §4 fits. What it costs: two taps for a word instead of one gesture; no position datum (Dr. Chen's CIBDAI attitude/activity item survives as the three energy chips *subdued · sleeping more · lively / restless* plus the absence chip — the same four positions without the false precision between them).
...
- **Once today's look is answered** the card collapses to its answered row, in ink, no tint, no tick, one `›` to the record: *Looked this morning · nothing unusual · 7:12* or *Looked this morning · sleeping more · 7:12*. **The subject of the row is the look, never the pet** — the row reports the owner's act. The question does not re-render until the next local day. A second look goes through the FAB (Door C).
- **Under a live safety card for that pet** (an `intake_decline` or `incident_red_flag` on screen, or `isAnimalNotEating` holding — the same predicate B-789's gate reads, failing closed) **the answered row withholds its words:** *Looked this morning · 7:12 ›*. The look is still in the record and on the report beside the meals; what Home refuses to do is draw "nothing unusual" one card below "Call your vet today" (the adversarial pass, gap 1b; the fold spec's veto of absence copy as a zone line, one synonym away). Symptom-class words still render on the row.
...
A look is always an attribution act (C-9: the card names its subject; there is no "not sure which" for a look — you looked at *a* cat). One card, both names as a pet row (*Pixel · Juniper — tap a name*); tapping a name opens that cat's grid; a cat not tapped is unrated, and that missingness is informative *per pet* (Data). Never two mandatory forms, never "you haven't looked at Juniper". **The leaf a word proposes is written to the look's pet by construction (T-7)**: the proposal's insert takes `look.petId`, never the active pet, so CUL-807 (the found-event attribution issue) is not a gate here — a look is witnessed and named, and the leaf inherits both. Pinned by a two-pet test in DL-3 and DL-4: a look on the non-active cat whose proposal is accepted writes the leaf to that cat.
...
11. **The first row is the absence chip and the intake router, at equal cost** (§3.1); the emergency door sits beneath it.
...
|---|---|---|---|---|---|---|
| Subdued | `subdued` | Lying about; slow to get up for the things she normally gets up for | Energy · `energyBehavior` | Existing leaf → **proposes "Log lethargy?"** (answered, T-6) | The word and its days; the lethargy row if accepted; the answer | calm · quiet (as a verdict) · settled · mellow · resting |
...
| Third eyelid showing | `third_eyelid` | The whitish inner lid across part of the eye | Face · Ears & eyes (lands W4) | Structured observation | The word; flagged in the appendix line | — |
...
| *Worried about something more serious? ›* | — | — | **Door, not a word — beneath the first row** | §4.6; reads, never writes | — | — |

### §4.3 Dog — 28 words, one router, one opening chip, one door (`panting_rest` withdrawn pending Dr. Chen's signature — §4.6)

| Chip (owner label) | Key | The video test | Look group · family | Disposition | Report treatment | Never beside |
|---|---|---|---|---|---|---|
| Subdued | `subdued` | Lying about; not getting up for the things he normally gets up for | Energy · `energyBehavior` | Existing leaf → **proposes "Log lethargy?"** (answered) | The word and its days; the lethargy row if accepted; the answer | calm · quiet (as a verdict) · settled · chilled |
...
| *Worried about something more serious? ›* | — | — | **Door, not a word — beneath the first row** | §4.6 | — | — |
...
- **Where:** a new seeded card, *The daily look* (working name), per pet, placed **after the intake cards** (never above the evidence it could seem to argue with — the adversarial pass, gap 2); a detail screen through the existing metric-detail doorway (`MetricDetailScreen`).
- **The card's first line is always the denominator:** *Looked on 24 of the last 28 days.* C-3's "render coverage as the un-logged days only, and nothing when fully covered" was written for a denominator the app controls; the look's is self-selected, and suppressing the line at full coverage removes the only cue on the day the reflex risk peaks (the adversarial pass, gap 5). So the line always prints, and prints the un-looked count beside it when there is one.
...
- **The withheld state (the floor's item 12):** while the pet's intake needs attention, the absence and positive rows are replaced by one line — *Withheld while Pixel's intake needs attention — her looks are on the report, beside her meals.* — and the symptom-class rows stay.
...
_Filled at the session's close from the reviews run: `adversarial-reviewer` (§4.9 — FAIL on the first draft, twelve gaps folded), `pm-feature-review` as Jordan and as Sam over the mock's flows. The falsifications tried are named in §4.9 and in the session record._
