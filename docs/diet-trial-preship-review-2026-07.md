...
Full chair transcripts: session record `docs/sessions/2026-07-27-b417-preship-review.md` and the PR that carries this doc. RLS chair: **PASS**, nothing ship-gating (two hardening items → B-541). Cold read: both artifacts **NOT READY** (Biscuit narrow; Miso blocking).
...
- **B-529** protein derived-from relation + primary↔set write consistency + antigen silence rule (R7; own robust PR)
- **B-530** refusal lane survives identity misses; refusals never re-render as off-diet exposures; `weightDuringTrial` decoupled from the refusal branch (`render.ts:1706`)
- **B-531** G2 leak fixes (`render.ts:3760/3776/2839`) + no-trial section rename (R2)
- **B-532** render honesty: Appendix E intake itemisation un-flag-gated (the legend's circular pointer — `render.ts:3220-3249`); "Ran its course" checked against `targetDurationDays` (`render.ts:1897`); Appendix D dose dates + unlogged-medication caveat; the unequal-denominator trend delta (`render.ts:2262-2292`); secondary cold-read items
- Then: fresh `vet-report-cold-read` on re-rendered artifacts → redeploy.

**Bucket B — gates the TestFlight build cut** (R1):
- **B-533** card viability wiring: the refusal state (consumes `trialDietRefusal`), the intake-rating teach line, states 3/4 + record-and-continue exposed (pairs **B-474**), free-fed forbidden-claim removed **with its locking test flipped** (`dietTrialCard.test.ts:843`), start-date field on the default path (R3), §10 S3 coverage clip parity with the report (**B-537**), 7a verdict line conditional (R4)
- **B-534** freshness pair: just-ended-trial report race (await/flush gate before "Open vet report"); Home strip invalidation (`bumpHydrationTick` on trial writes)
- **B-535** start-modal → food-capture round trip resumes the half-filled form
- **B-536** outcome-facts meal-day guard fix + outcome-optional copy (R4)
- **B-538** grace windows 90/30 (R5)

**Bucket C — before the trial surfaces are demoed together:** **B-539** `ask` `since_trial_start` zoned day math + `dietTrialStatus` status guard (G5 parity; distinct from B-517) · **B-540** ghost-active-trial takeover (pairs B-452) · **B-541** RLS hardening (food_item_id ownership into the 041 trigger; rule 041's two deferrals) · **B-544** StartTrialModal + useDietTrial test coverage (the sole `diet_trials` write path has no test file).
