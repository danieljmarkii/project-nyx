...
| B-010 | Remote→local sync for `event_attachments` and `vet_visits` (+ visit attachments) | The v1.19 hotfix added `downloadRemoteData(petId)` but only covered `events` and `meals`. A second device signed into the same account will see the *timeline* now, but photos attached to events and any vet-visit history are still missing because their sync is upload-only. Pattern is identical to the events fix (watermark + upsert on id with `WHERE synced = 1`); only complication is downloading the actual photo bytes for attachments (probably via signed URL into local cache, similar to existing photo viewers). Lower priority than events because attachments are rarer and the report PDF will pull from Supabase Storage server-side anyway. | Next | 2026-05-17 | After v1.19 ships; before TestFlight | Open |
| B-011 | Wire up Jest test harness | Two `*.test.ts` files exist in the repo (`store/attachmentStore.test.ts`, `lib/sync.test.ts`) but `npm test` is not defined in `package.json` and `jest` is not in `devDependencies`. Tests document intent but never run. Install `jest`, `jest-expo`, `@testing-library/react-native`, add a `test` script, and confirm both files pass. CLAUDE.md DoD already requires `npm test` before pushing test-touching diffs — that gate is currently unenforced. | Next | 2026-05-17 | Definition-of-Done enforcement | Open |
| B-012 | Onboard wife to the app via TestFlight | Wife-onboarding attempt via Expo Go failed (see v1.20 retraction — Expo Go ignores EAS Update bundles when `runtimeVersion` is set). Right path: once Apple Developer enrollment lands, `eas build --profile preview --auto-submit` to TestFlight Internal Testing, invite wife as internal tester, she installs Nyx like a regular app. After that PR #25 (v1.19 remote→local sync) can finally be validated on a real second device, and the daily flow becomes `eas update --branch preview` + cold-open Nyx on both phones. Sub-tasks: bundle identifier decision, App Store Connect app record, EAS Build profile review, first build, TestFlight submission, internal tester invite, end-to-end QA of v1.19 sync. | Now | 2026-05-17 | Apple Developer enrollment approval | Open |
