...
| **D3** | **The rundown block's window.** **Past 12 months shown, earlier courses folded behind a count** ("3 earlier courses, over a year ago") — speakable for Sam's chronic cat, complete for the lifetime question (the profile section + report carry the full named lifetime). | **RATIFIED 2026-08-04** — PM confirmed the provisional at PR 4's handoff ("go with your recommendation"); PR 4 (#589) built exactly this, so no change followed. |
...
3. **Rundown medication-history block** (`lib/rundown.ts` + `app/rundown.tsx`) — "Medications — current" (existing) + "Medications — past 12 months" (D3 ratified), rows written to be read aloud: drug name first, then speakable dates. Deterministic, offline, same derivation as §02. **Shipped via #589.**
...
| **4** | Rundown medication-history block (D3: 12 months + folded earlier count). **✅ shipped via #589** — D3 ratified at the handoff. | `lib/rundown.ts`, `app/rundown.tsx` | `clinical-guardrails` + `nyx-voice`; offline-path QA (zone-stable); `code-reviewer` fix-before-merge closed (the courseRecencyMs B-441 trap). |
