...
- **The in-report chart is a `generate-report` renderer change → B-730**, riding the B-494-held redeploy (never its own deploy, per the standing rule). Until B-730 ships, frame 3 must show the report exactly as the deployed function renders it; the enriched frame 3 is recaptured when it lands.
