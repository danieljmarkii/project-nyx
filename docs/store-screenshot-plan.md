...
**Status:** 🌱 **Design COMPLETE — every D-SS call ruled (§0.5). Export pipeline BUILT (#625): `scripts/store-screenshots/`** (round-5 template at true 1320×2868, real Geist/Newsreader, `--draft` + capture modes; see its README for the capture workflow + `hero-crops.json`). Remaining is production: captions finalized with step 13's listing (edits only the template's `FRAMES` strings), then capture — gated on step 11. **DB-1 ruling (PM, 2026-08-11, demo-plan review):** submission #1 captures with **`signal_design_v2` OFF everywhere** — the hero uses the plain-cards fallback; the enriched hero (dot-lane receipt) and the B-742 report chart are **v1.1 recaptures**, no longer submission gates.
...
- **(2) The constellation** — the whole set in the night; the dot-lane receipt drawn as sky (beef dots, vomit stars, hairlines) over one glowing real card. Most distinctive; amends C's ground split (night everywhere) and spends the most "real product" proof.
...
**D-SS4 — RULED 2026-08-09 (PM): return to the round-3 bezel template, enriched.** The bezel-free concepts are closed (recorded in the mock). Round 5 applies the three asks: **(1)** the hero's insight card carries the **B-721 dot-lane receipt** (the new Signal dashboard's shape A; S1 respected — the safety card stays plain, plainness is the severity signal); **(2)** the report frame gains a **by-day symptom chart**, trial start anchored; **(3)** Cooper renders with a **species-matched illustrated avatar**. **Frame 6 RULED 2026-08-09: 6A — the full Home** (the trial-picker frame is dropped from the set). Every design decision on this track is now closed; what remains is production (captions with step 13, B-742, v2 presentability, export, capture).

**Two product dependencies created by the ruling (flagged, never silently absorbed):**
- **`signal_design_v2` on the demo account — SUPERSEDED for submission #1 (DB-1, PM 2026-08-11).** This carve-out conflicted with the demo spec's D9 (never allowlist the review account), and the PM ruled **D9 governs: v2 stays OFF everywhere — capture and review — for the first submission**; the hero ships the plain-cards fallback below, and the receipt-rich hero is a v1.1 recapture once v2 is GA. The carve-out's one enduring rule survives as the invariant: **never one without the other** — capture-on/review-off is a Guideline 2.3 exposure and is forbidden in every future round. *(Original carve-out text, for the record: the flag would have gone ON for capture AND review once SR deemed v2 presentable.)*
- **The in-report chart is a `generate-report` renderer change → B-742** _(filed as B-730; renumbered at wrap — the ID was taken on `main`)_, riding the B-494-held redeploy (never its own deploy, per the standing rule). Until B-742 ships, frame 3 must show the report exactly as the deployed function renders it; the enriched frame 3 is recaptured when it lands.
- The avatar needs **no product change**: the illustration is produced as an asset and uploaded as Cooper's pet photo during step 11 phase D (added to §2's prerequisites).
...
5. ~~**Seed addendum flagged to the B-271 seed-script session**~~ — **resolved by the demo spec's v2 refresh (2026-08-09) and verified in the 2026-08-10 panel review**: the spec now seeds the full B-417 lifecycle including the dated `diet_trial_foods` membership row and `target_protein`, so the TrialStrip renders "Venison trial" and the picker's pinned trial zone works. *(This note predated the v2 spec; kept struck-through for the record.)*
