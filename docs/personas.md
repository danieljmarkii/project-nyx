...
**Active responsibilities:**
- **Reconcile** each Linear issue's `state` against what actually shipped (commits, merged PRs, `docs/sessions/` records) — an item that merged is `Done` (with the PR linked), not `In Progress` / `Todo`.
...
4. **What working file is bloating?** Check the state surfaces against their budgets — the **Linear board** (no stale `In Progress`, aged high-priority issues triaged — a `backlog-groomer` pass), CLAUDE.md's Open Questions (resolved rows not piling up), and CLAUDE.md itself. `STATUS.md` and `docs/backlog.md` are both exempt now for the same reason from opposite directions: the backlog is frozen, and `STATUS.md` was reduced to a pointer card no session writes to (2026-08-22), so neither can accrete. The failure mode here isn't a missed bug — it's *accretion*: files that only grow, optimizing for "lose nothing" over "stay legible," until reading them costs more than the work. If a file has outgrown its budget, prune it this retro. (The 2026-07-19 retro that added this check found STATUS.md at 210 KB / 26 K words and the backlog at 403 KB / 242 open rows — both working files that had become archives. STATUS.md was pruned that retro and had regrown to **239 KB** by 2026-08-22, which is what proved that a budget without a structural fix only buys time.)

**Trigger (so it actually fires — it hadn't been):** the retro is due whenever any of these is true — a build-phase boundary is crossed; a working file has outgrown its budget; the last retro is more than ~10 `docs/sessions/` entries back. `/wrap` should flag "a process retro is due" when it notices one of these, and `/kickoff` should surface it. Don't wait for pain.

Record the outcome in the session summary's "Decisions Made" and apply durable changes to CLAUDE.md / this file immediately (Tier 1). **Retro log:** 2026-07-19 — first run of this ritual (PM-initiated workflow reflection). Outcome: instituted the STATUS.md size budget + `/wrap` prune-while-you-prepend teeth; the two-tier doc-versioning discipline (living vs. frozen, header-not-filename versions); demoted `nyx-schema-v1_0.sql` to a snapshot pointing at migrations; filed the backlog archive-split, CLAUDE.md trim, Open-Questions consolidation, and CI-workflow items for PM sign-off. Full write-up: `docs/workflow-retro-2026-07.md`. — 2026-09-11 — second run (PM-initiated: "take a hard look at our backlog grooming workflows"). Seven isolated lenses, each red-teamed, then a net-out critic. Outcome: five live defects found and verified (the groomer reads a 19-commit-stale `main`; the abandoned-claim detector cannot fire against 779 never-pruned branches; step 0 exits non-zero on a healthy repo; this ritual's own trigger was specified 2026-07-19 and never built — `grep -rin retro .claude/` = 0; CLAUDE.md contradicts itself on its first screen); seven laws (L1 a queue with a mandated ADD and no mandated REMOVE is the old checklist with a new backing store — `Waiting on PM` is at 97; L2 **prose fires approximately never, a guard or a cron has zero recorded misses, and there is no third tier**; L3 a cap on COUNT with no cap on SIZE is not a budget; L7 a detector whose only evidence is the existence of a ref is not a detector); the `Waiting on PM` → `Needs PM` workflow-state ruling; and the fan-out answer, *fan out the read, serialize the write*. Full write-up: `docs/workflow-retro-2026-09.md`.
