...
**The moment (Home, mock A2).** Inside the window — **five days** before through the day of — the appointment strip renders **under the Signal, in the trial strip's register** (`index.tsx:182–200`): a context strip, never an insight. **Rule:** a live safety or intake card always renders above it, its own ask intact; no Signal string is changed, dated or re-phrased because an appointment exists; the strip never gains urgency styling. Two doors: **Get ready** and **Add a question**. The report send lives inside Get ready.

> **⚠ RULED 2026-09-11 (CUL-903, VV-5 build).** *Add a question* is a **DOOR into Get ready**, not a sheet on Home. The register rule (`docs/nyx-med-strip-requirements.md` §0.1) bounds what Home may write, and D1's second clause forbids a control that opens a FORM there. Two halves, ruled separately: ***It didn't*** writes `cancelled_at` **from the strip** — a third Home write class, PM-approved as a Tier-2 amendment because it passes D1's own test (the app is describing the row on screen as it asks about it, and the control opens nothing); ***Add a question*** navigates, because typing is a form. **Home gains a confirmation and still carries no form.** The questions still save to `vet_appointments.questions` — from Get ready, which is where mock B1 already draws them. Enforced by `guards/homeWrites.test.ts`, which red on the first companion node mounted on Home and is what sent this to the PM before the strip was built.
...
> **⚠ BUILD CALLS 2026-09-11 (CUL-903), recorded because each narrows a sentence above.**
> 1. **The cap of four binds the RECORD's rows, not the owner's questions.** Capping the merged list would mean typing a fifth question deletes a Signal finding — the app discarding a safety sentence because the owner had a lot to ask. Rendering follows mock B1 exactly: one numbered list, headed *Worth raising* when derived rows exist and *Your questions* when only the owner's do.
> 2. **Safety findings sit ABOVE the cap.** The Signal's findings are unbounded server-side, and Principle 3 says safety insights "are never dropped to honor a layout cap". Below them the order is trial → course → weight → the remaining Signal findings, which is the issue's own source set with one change: a wedge owner at a recheck FOR the trial, whose Signal carries four benign findings, would otherwise open this page and find no mention of the trial.
> 3. **The weight row ships with NO duration threshold.** *"When no weigh-in is recent"* needs a meaning for *recent*, and a threshold in days would be a new claim over a new window on the one page whose entire rule is that it makes none (G6 / CUL-746) — with no constant anywhere in the app to mirror, this module would be deciding alone how long is too long between weigh-ins for any species, age or condition. Two **record-anchored** gates instead: no weigh-in at all (asked of the rundown's own tile), or the newest weigh-in **predates the last logged visit** — *the vet's own number is still the newest one*. The row prints a **date**, never a duration (C-19). It under-fires for a first-time owner, which is the direction this page may be wrong in.
> 4. **Get ready reads `readSignalCache` directly, never `useSignal()`** — that hook refreshes a stale cache through `functions.invoke('generate-signal')`, i.e. the Haiku phrasing call, so the obvious wiring would have broken AC 4's zero-model-calls promise on any cache older than 24h. The cache is a network read, so offline the Signal rows are absent while the local rows still build; the section **names the gap** rather than falling silent, because an owner who cannot tell "nothing standing" from "we could not look" reads the first and walks into the room reassured (C-12).
...
- **The seed migration — `0NN_vet_visits_config.sql`, at the next free number when VV-0 is built** (064 went to Noticed N-1's `looks`, #820, while this spec was being written; 065 is free as of 2026-09-10 — check `supabase/migrations/` first)**:** `INSERT INTO app_config (key, value) VALUES ('vet_visits', '{"enabled": false, "allowlist": []}') ON CONFLICT (key) DO NOTHING` (`063:88–91`) — seed-first, default nobody; cohort enablement (the PM's uid) is a later recorded config UPDATE, never baked into the seed; the App Review demo account is never allowlisted.
...
|---|---|---|---|
| **VV-0** | **The flag.** The seed migration (the next free number — 064 is Noticed N-1's) seeds `vet_visits` dark; the client key; the `BETA_REGISTRY` row; the `beta.tsx` case; the AC 0 snapshot harness. No consumer. | nothing — **start now** | Key name (`vet_visits` ✓ vs `vet_visit_companion`); whether the PM's uid is allowlisted at seed (no ✓ — a later config UPDATE, the 063 lesson). |
...
- *VV-0:* "Build VV-0 from `docs/nyx-vet-visits-requirements.md` §5.5 — the `vet_visits` rollout flag: the seed migration at the next free number (mirror 063 verbatim; 064 is taken by Noticed N-1), the client key, the registry row, the `beta.tsx` case, and the AC 0 flag-off snapshot harness. No consumer. Schema-isolated."
