...
**Version:** 1.1 (**BUILD-READY** — D1–D4 PM-ruled 2026-09-05) · **Date:** 2026-09-05 · **Owner:** Sr. Product Designer (design authority), Dr. Alex Chen (clinical conditions), Dir. of Engineering (the route + the motion contract) · **Track:** CUL-800 (parent; absorbs CUL-158 / B-029) → CUL-802 (PR 1, the route), CUL-803 (PR 2, the screen), CUL-804 (PR 3, the arrival); CUL-801 (the trigger race) blocks PR 1 · **Design authority:** `docs/culprit-incident-screen-mockups.html` round 2 (the artifact re-publishes to the same URL)
...
- **§5.1 Order.** Hero (unchanged, D3) → the record block (type label, date, time, confidence tag, **"{pet}'s record"** via `resolveRecordPetName`, never `activePet`) → **the read** → the observations → Food / Intake / Notes / the rest, as shipped.
...
- **§5.3 The observations ("What's visible").** A two-column label/value grid, `Edited` markers and the "Edited {date}" line as shipped. A `Keep it compact` control (expanded state only; never a swipe, never the face tap — the fold spec's rule) folds it to a one-line strip: the values and the count ("Yellow, foamy, bile · 4 findings"); the strip re-opens it. Persisted per pet per event, device-local, the Signal fold's store shape (`lib/signalFold.ts`), **wiped by name in `wipeLocalSession`**. Editing is unchanged: the owner's edit stays the more-trusted value and a re-analysis never clobbers it. A `Blood: none visible` row is a fact about this frame, never a wellness claim; CUL-531 tracks dropping it under a contextual escalation.
  - **§5.1a ⚠ Correction (CUL-803 build): this does NOT land CUL-660.** §5.1 originally said the record-block line "lands CUL-660". It does not, and the claim should not have been written without reading that issue. CUL-660 **settled** that the pet's name goes in the **header bar** — B-550's reasoning, that the bar sits outside the `ScrollView` and is therefore still on screen when the phone is turned around over a scrolled record — and holds an **open R1 decision brief** on the form it takes there (`Mochi's vomit` / `Mochi · Vomit` / a two-line header), with its own draft PR (#784) and mock. The line this spec specifies is inside the scroll and answers a different question: it says whose record this is *while you are reading the record*. Both are wanted; neither replaces the other. What CUL-803 does change is a **premise** of R1's truncation argument — "the pet is stated nowhere else on the screen" is no longer true — so the brief is re-raised on CUL-660 rather than silently inherited.

  - **§5.3a Two rules the build added (CUL-803, from the adversarial pass — binding).** ① **An escalation's facts do not fold.** The named slots are spent in row order and every builder pushes the descriptive rows first, so on a fully-populated read Blood, Mucus and Foreign material are *always* the rows compressed to a count — which on a `worth_a_call` puts the verdict on screen and every fact justifying it behind a tap, on the surface D3 exists for. Rather than rank the rows clinically (a second symptom predicate, which the house forbids), the acute case simply does not fold. This is the Signal fold's own DF-2 shape. ② **The record re-opens a fold.** `Re-run analysis` renders *in* the folded state, so a re-analysis (or a late realtime resolution, or the add-photo re-read) lands new observations into a folded grid. The fold releases when the observation set changes; the baseline is adopted at mount, so an earlier session's fold is honoured on arrival and only an in-session change re-opens it. ③ The strip's named half and its count must draw on one population: a row's value can itself be a list (`contents`), so the strip re-separates the inner list (`observationStrip.ts`) or four rows render as five comma-separated items beside a count of four (C-3). ④ When the line must give, the **count** is pinned and the named values yield — the count is what makes a partial enumeration honest.
...
`guards/accentOnLight` · `guards/geistRollout` · `guards/recordPetName` · `guards/haptics` · `guards/ownerFacingCopy` · `guards/reversePath`.

⚠ **Correction (CUL-803).** This section originally named "the wipe-set scan in `hydration.test.ts` for the fold store key". That scan covers **SQLite tables only** (`LOCAL_WIPE_TABLES` / `NOT_WIPED_ON_SIGN_OUT`); it cannot see an AsyncStorage key, so it never covered this one. The actual coverage is the hand-written assertion in `lib/session.test.ts`, which reads the key back through the module's own reader after `wipeLocalSession()` — the same shape the Signal fold's entry uses. **There is no machine guard forcing a new AsyncStorage key onto the wipe path**, which is a standing convention gap, not this track's.

**Also added by CUL-803:** `components/event/IncidentReadCard.tsx` and `components/event/ObservationGrid.tsx` join `guards/haptics.test.ts`'s `ALWAYS_SCANNED`. Extracting the read's rendering out of the two analysis sections moved the surface that paints a `worth_a_call` beyond the guard's marker heuristic, and both scanned files stayed green over it.
...
| 1.1 | 2026-09-05 | §5.3a added and §6 corrected during PR 2's build (CUL-803, #806) — the fold's two missing rules (an escalation's facts do not fold; the record re-opens a fold), the strip's two honesty rules, and the §6 pointer at a `hydration.test.ts` scan that structurally could not cover an AsyncStorage key. All four came from the `adversarial-reviewer` pass, which returned FAIL on the first cut. |
