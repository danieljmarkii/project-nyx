...
### Phase 2 — Publish + share + photos (functional → operational; the first unauthenticated path)
*The minimum viable way to get it to a vet, plus incident photos. The expensive/irreversible bit — do it after the vet reaction.*

- **PR 5 — `generate-report` Edge Function + share token + `view-report` route.** I/O shell (caller-auth → **ownership re-check** → window → pull → `report.ts` → `render.ts` → store snapshot → insert `vet_reports` → token/url) + the public token-gated `view-report` (expiry/revocation enforced) + **all §8 privacy controls**. **`rls-privacy-reviewer` (mandatory — first unauthenticated path).** **Precondition: B-044.** Deploy via the B-082 MCP path.
- **PR 6 — In-app generate → share.** Replace `app/report.tsx`: generate (default cascade) → **copy link + Save/Print PDF via the system share sheet** + the §6 empty/sparse/expired states. *(Enough to hand a vet a live link or a PDF — the "simplest path" of the delivery ladder.)*
- **PR 7 — Incident photos + revocation** *(point 8 + B-143)*. Photos on the report **split by type** (everyday streamed/revocable/excluded-from-PDF; safety-flagged permanent record copy) with the §8 photo controls; owner "kill this link" revocation. **`rls-privacy-reviewer`.**

> **➜ End of Phase 2 = the v1 the PM considers done:** full report, functional, operational, shareable, with photos.
...
**Consumer ownership map (so nothing is double-built):** B-117 PR 10 → PR 3 · B-102 PR 6 → PR 4 · B-040 rendering → PR 4 · B-010 → PR 3 · B-186 weight → PR 4 · phenotype (point 7) → PR 3 · photos (point 8) → PR 7 · B-143 → PR 7 · B-023 PR 5 → PR 10 · B-144 → PR 11.
