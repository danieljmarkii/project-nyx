...
**Status:** Build-ready v2 — updated 2026-07-02; **photo decision revised 2026-07-04**; **lifetime medication-history table added 2026-08-04** (B-140 PR 5, D2 — §3.8 / Appendix D); **Signals v2 timing types adopted 2026-08-21** (CUL-564 — §3.8 timing line). | **Owner:** Step 9 build sessions.
...
>
> **v2.2 update (2026-08-21) — Signals v2 timing types (CUL-564).** Signals v2 GA'd on the Signal surface (CUL-546), and `detection.ts` is shared with `generate-report`. The report had been pinned to the pre-v2 timing taxonomy (`composeV2:false`) as a stopgap, which silently **dropped** a post-prandial (⑤) finding whenever it merged with a same-symptom empty-stomach lane (L1) into a `timing_story` the report ignored. The report now **runs the full v2 composition** and its §3.8 associational timing line renders the v2 types: **the post-prandial band (⑤, ≤30 min after eating, unchanged), the empty-stomach band (L1, ≥6h after eating), and the merged `timing_story` when both fire** — all band-named / associational only, never a syndrome name (bilious/BVS is the vet's inference). **Two v2 lanes are deliberately EXCLUDED from the report (PM + Dr. Chen, 2026-08-21):** **`trial_response` (L2)** — the report's dedicated diet-trial section (§1/§3.8) answers "is the trial working?" at higher fidelity, so a second count block would duplicate it and risk a contradicting denominator; and **`gap_shortening` (L4)** — a sub-floor watching row whose confidence the **§8.5 `Established`-only discipline** keeps off the clinical report (a 4-episode monotone run reads as a *finding* on an authoritative page), and **Appendix A + the §3.5 trend chart already carry the cadence at full fidelity**. Both remain owner-surface signals. This finished GA-3's cleanup: the `composeV2` toggle + the legacy pre-v2 composition fork are removed from `detection.ts`.
...
8. **Diet, feeding, medications & supplements** — diet (trial *or* current) · **free-fed "Intake not directly observed" (B-040)** · feeding method + meal-completion (meals-only) · **human-food line (B-102)** · treats summary · **medications + adherence (B-117)** with the concurrent-change note · **supplements as concurrent interventions** · the associational vomit-timing finding — post-prandial (⑤, ≤30 min) and/or empty-stomach (L1, ≥6h) bands, merged into one line when both fire (**CUL-564**); band-named only, never a syndrome name; L2/L4 excluded (v2.2 note) · pointer to the WSAVA appendix · pointer to the **lifetime medication-history table** (in the appendix zone, beside Appendix D — **B-140 PR 5, D2**, below).
...
- **Medication history (lifetime) — the D2 table (B-140 PR 5, added 2026-08-04):** immediately **above Appendix D**, a compact **window-ignoring** table — **drug · dates · course · doses logged** — listing **every** medication ever logged (active *and* past), so the vet's *"has she ever been on steroids?"* question is answerable from the report a 30-day window would otherwise hide. It **ignores the report window on purpose** but reads the pet's whole record (regimens are unbounded; the dose set is pulled untrimmed — the same DB pull, mapped without the lookback trim), so it is genuinely lifetime, not a second appendix. Three invariants bind it, enforced by tests over the derivation and the copy (not review): it derives from the **one shared `lib/medicationHistory.ts` predicate** every B-140 surface reads — a count here can never contradict the app or Appendix D (**H4**); it states an **ending ONLY from an owner action** — a course that merely went quiet reads its *last dose*, never "completed"/"ended" (**H1**, the B-422 stale-active rule); and it carries **no adherence % or grade**, only counted facts like "26 of 28" (**H2**). A coverage note ("Lifetime of the record — since …") and the H1 disclosure lead the table, **before** the data (B-494 — a load-bearing disclosure a skimmer must apply is not left to a footnote). Design authority: **mock §05**.
- **D — Medication log:** per drug — regimen, doses logged/scheduled, adherence narrative (honest about unconfirmed doses). Supplements listed in C. **This is the WINDOWED dose-level detail beside the lifetime table above** — the lifetime table is the "what, ever?" overview; Appendix D is the "how was the current course dosed, in this window?" detail; the lifetime table's note points here.
