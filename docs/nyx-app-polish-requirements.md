...
- **Wave 0 — foundations (run first, parallel-safe):** CUL-604 (`lib/haptics.ts`, §5.6 — unblocks the completion chain + CUL-601's tap) · CUL-605 (`ThemedText`, §7). **Both shipped 2026-08-22 in parallel sessions** — CUL-605 via #699, CUL-604 via #700 — so the completion chain and the Geist sweeps are unblocked from day one. Wave 0 is closed.
