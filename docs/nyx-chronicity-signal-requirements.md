...
**PR-1 adversarial-reviewer findings (2026-06-26 — verdict PASS, not a merge-blocker):**
- **D2 sensitivity has a measured false-negative cost the noise gate cannot see.** The reviewer reproduced the sweep (minEpisodes 4→10.3% / 5→4.3% / 6→1.5% on the ~2/56d model) AND measured the other side: `minEpisodes 6` SILENCES a real **once-a-week-for-5-weeks** course (5 episodes) and an **every-other-week-for-6-weeks** course (4 episodes). The §7 #14 property test only measures false-positives (noise), never false-negatives (missed real chronic courses), so it *cannot adjudicate* 5-vs-6. Also: the gate "passes" only at the assumed ~2/56d base rate — at ~3.5/56d, `minEpisodes 6` fires ~12.4%, so the floor's adequacy is a function of an *assumed* rate, not data. **→ D2 5-vs-6 routes to Dr. Chen with the false-negative side explicit** (6 = credible-surface; 5 = catch-the-once-weekly-course, defensible because the safe error direction for a safety lane is toward firing). A property test on *real low-count chronic courses* (does the floor catch them?) should sit alongside the noise test before the floor locks. **PM action item.**
- **activeWeeks "barbell" bug (→ PR 2 hardening, B-188).** `activeWeeks = floor((now − onset)/7d)` is **now-anchored**, so a 3-consecutive-day cluster lands in 1 OR 2 buckets depending on the wall-clock phase relative to `now`. Consequence: (a) a **two-cluster barbell** (3 stale + 3 recent, ~6 quiet weeks between) can reach `activeWeeks = 3` because one cluster straddles a bucket edge — the exact two-endpoint/one-cluster shape §4.2/§4.3 claim the floor *excludes*; and (b) identical data flips fire↔silent across calendar days (jarring on a safety surface). Safe-direction (over-fire, never reassurance), only the marginal shape at exactly activeWeeks=3 — but a true correctness gap vs §4.3's stated intent. **Fix in PR 2** with a calendar-independent, straddle-robust distribution measure (the simple re-anchors — `firstOnset`-anchored or fixed-UTC-week — kill the *non-determinism* but a consecutive-day cluster can still straddle a fixed 7-day boundary, so the real fix is a distribution measure that counts distinct *bouts'* weeks phase-stably). Proposed §4.2 spec edit flagged. PR 2 is adversarial-mandatory anyway.
