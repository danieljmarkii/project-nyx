...
**A1 — TodayZone v2 (resolves B-673; revised after the PM's "new UI is fine — make it beautiful and informative" clarification, 2026-08-15).** The zone keeps its job (today, capped, on Home) and gains the recap's language: a **recap band** where its header sat — `TODAY SO FAR` + the compact day lane (the spine's horizontal cousin: category-tinted dots at real times) + an honest count line (`2 meals · 1 dose logged`) + `Full day ›` into the night surface — with the capped event rows continuing beneath exactly as today. Principle 3 held by construction: the Signal still leads Home; this is TodayZone *evolving*, not a new card — facts and a door, no verdicts, no badge. The zero-log day renders the lane empty beside TodayZone's existing empty nudge (nothing manufactured). The lane and the spine share one visual language so Home's glance and the ritual's read are the same thing at two sizes. (The round-3 "retarget the existing tap" version is superseded; A2/A2b ride unchanged.) Mock: round 5.
...
**B1 — The hero is the destination, in the `ValuePreview` grammar (round 5: picture-forward).** The Landing's learn-more pages (`components/onboarding/ValuePreview.tsx`, B-251 PR 5) already solved this exact problem — pitch a feature by showing a *miniature of the real surface*, then a display-face headline, then one calm subline. After the PM's second reaction ("still boring — bring the chart in, kind of cartoonish"), the hero is a **mini-spine**: the day timeline in miniature — bigger nodes, no card chrome, a warm generic sample day (breakfast · a dose · dinner; never the owner's data — the primer must render before any logs exist) — simplified but built only from real product language (the `ValuePreview` Eng note: never preview UI we don't ship). Body copy cut to one sentence; total word count roughly half of round 4's. Two copy directions drawn:
...
- **C1 — The day spine (revised at round 5 after the PM's "boring / bring the timeline back" reaction).** The timeline IS the event list: the day renders as a **vertical timeline** — time labels left, category-tinted nodes on a connecting thread, each event's title + intake/adherence beside its node, every node a doorway. One visualization, no lane-plus-list duplication; gaps between nodes read as real gaps in the day; the sequence (a refused breakfast two hours above a vomit) is visible *as sequence* without a verdict word. Sub-lines carry record facts only ("Trial diet", "Photo attached"). The round-3 horizontal arc survives as the **compact glance form** on TodayZone v2's band (A1) — same dots, same tints, two sizes of one language.
...
- **The delivery question is the real decision.** The engine runs server-side on a request-driven 24h cache — nothing server-initiated exists (D2). Three paths: **(i) server push** — Part 2, finally forces the long-open provider OQ; **(ii) background app refresh** — `expo-background-task` (BGAppRefreshTask): the app wakes opportunistically, calls `generate-signal`, diffs finding identities against a local last-notified ledger, and schedules an *immediate local* notification if a new safety finding appeared. No push provider, no APNs entitlement; costs the background-fetch capability (config plugin) and iOS-controlled timing (hours-scale, same-day — acceptable for patterns that emerge over days, and the body never claims immediacy). **(iii)** compute-at-9pm piggyback — rejected (the owner is already opening the summary). Path (ii) is the team's lean for v1 and needs an engineering spike (reliability of BGAppRefreshTask under iOS budgets) before it's promised.
- **Drawn (mock round 5, per the PM's ask):** the lock-screen envelope — title `Signal`, body `Something new in the record is worth a look.` — plus the firing-model table on the frame: trigger (new safety-lane finding, identity-keyed, once ever), delivery v1/v2 as above, caps (one envelope even for multiple findings; the unsolicited budget), tap → the Signal zone with the finding expanded. Because only safety-lane findings fire, the channel itself carries the weight — the calm body is load-bearing, not timid.
