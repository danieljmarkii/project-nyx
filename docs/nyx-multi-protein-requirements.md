...
_(v1.1, 2026-07-24 — build-session rulings folded in: **D3a** the Class-A/Class-B merge rule (amends D3, from B-414); **D9** the "Other" typed-escape normalization + its inline disclosure (B-412); **D10** the protein-set completeness gate (B-413). §11 gains slice 0 and the B-416 backfill.)_
