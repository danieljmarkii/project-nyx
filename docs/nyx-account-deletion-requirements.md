...
- **FR-8 — discoverable, not buried.** A clearly-separated **"Account"** section at the bottom of the profile tab (`app/(tabs)/profile.tsx`) with a destructive "Delete account" row. Theme tokens only; `hitSlop` to ≥44pt.
...
- **Kickoff prompt:** _"Build B-039 PR 2 per `docs/nyx-account-deletion-requirements.md` §12 — the client deletion UX (FR-8…FR-12): an Account section at the bottom of `app/(tabs)/profile.tsx` with a destructive type-to-confirm `DELETE` flow (no dark patterns), an offline guard, nyx-voice copy, and a post-success `supabase.auth.signOut()` that reuses the B-054 FR-9 local wipe. Verify a full delete on a throwaway account on-device."_
