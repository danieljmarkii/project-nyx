...
**Version:** 1.1 (**BUILD-READY** for PR 1 and PR 2 — the PM ruled DF-2 / DF-3 / DF-4 on 2026-09-03; v1.1 folds in the acute-class fold, the face control, and the two new asks: the fold motion §12 and the card-design round §13) · **Date:** 2026-09-03 (v1.1) · **Owner:** Sr. Product Designer (design authority), Dr. Chen (clinical conditions), ratified through the fold mock round 1 · **Track:** CUL-695 (The Living Signal), direction **F3 "Seen & fold"**, re-scoped by the PM on 2026-09-03 as **Home v1** — the minimize/expand, rolled out to all accounts, built now; **Home v2** (the re-imagination) is the *Home Redesign — Conference Spike* project and is not this document.
...
| DF-10 | The fold motion ("design delight") | PM ask, 2026-09-03: *"as these cards are expanding and contracting, let's add some design delight."* Spec'd in **§12** from the Designer's round-2 pass: **the rail is the continuous thread** — words leave first, the box closes around the line, the strip arrives with the box; unfold is the reverse with a soft settle; identical physics on every class (S1 lives in what the strip says, not in how it moves); owner-caused only (FS-9); no haptic, no wash, no bounce on close, no loop; reduced-motion = crossfade only. Drawn as a tappable frame in mock round 2. Ships as **PR 3** (CUL-788). | Spec'd — PM approves or asks for a variant on CUL-789 |
| DF-11 | The card design refresh | PM ask, 2026-09-03: *"if there's anything we can do to improve the design of the cards, let's explore those options."* Three directions in **§13**, drawn side by side in mock round 2: **A "Quiet foot"** (the meta row de-pilled, one control row), **B "Margin rule"** (the rails become one ruled margin; row hairlines go), **C "Picture first"** (the receipt leads on benign cards — **vetoed**, it re-opens S4/S10 and breaks the fold's compression logic). Recommendation: **build B carrying A's foot**. The pick is the PM's (CUL-789); the build is **PR 4** (CUL-790). | Drawn — PM pick pending |
...
| `intake_decline` (`consecutive_low`) | `Eating less than usual — {ask}` where `{ask}` is the card's own verb: `call your vet today` when the sentence says so (the feline 3-day floor), else `check with your vet` | `sampleLine` ("3 days below the usual, across 9 recent meals") |
| `intake_decline` (`refused_normal_food`) | `Refused the usual food — {ask}` (same verb rule) | `sampleLine` |
| `incident_red_flag` (blood) | `Blood in a {vomit\|stool} photo — call your vet` | `sampleLine` + ` · last {Mon D}` |
| `incident_red_flag` (foreign material) | `Something unusual in a {vomit\|stool} photo — call your vet` | `sampleLine` + ` · last {Mon D}` |
...
| **PR 4 — the card design refresh (DF-11, §13)** | The PM's pick from mock round 2, built with tokens on the existing anatomy; S1 plainness by contrast preserved; `pm-feature-review`. | the round-2 pick |
...
PR 1 → PR 2 are sequential (PR 2 flips the class gate PR 1 ships closed); PR 3 can run parallel to PR 2 once PR 1 merges (it touches `InsightCard`'s transitions, not the strips' content); PR 4 waits on the round-2 pick. None touches `supabase/functions/`. The two v1.1 items are engine work and ride the normal `generate-signal` deploy path (currently unblocked — not under CUL-19 / CUL-557).
...
### 12.1 Fold (face / expanded → strip) — `durationSlow` 400, composed 150 + 250

| t (ms) | What moves | How |
|---|---|---|
| 0 | Tap `Keep it compact`. No press flash beyond the default. | — |
| 0 → 150 (`durationFast`) | The body — sentence, receipt, meta row, evidence, control row — opacity 1 → 0, translateY 0 → −4 (`space0_5`), drifting toward where the clause will sit. | `Animated.parallel([timing(opacity), timing(translateY)])`, `Easing.out(Easing.quad)`, native driver |
| 150 → 400 (`durationMedium`) | Row height expanded → strip. The rail (same View, `alignSelf: 'stretch'`) shrinks with it. Every sibling below rises in the same transaction — Today and Trend come up into view for free. | `LayoutAnimation`, `update: easeInEaseOut` |
| 150 → 400 | Strip clause + count line + chevron opacity 0 → 1 as ONE node — no stagger between the words. Legible by ~275ms, the ease's midpoint. | `LayoutAnimation`, `create: { easeInEaseOut, opacity }` |

### 12.2 Unfold (strip → face) — `durationSlow` 400, one stage

| t (ms) | What moves | How |
|---|---|---|
| 0 → 400 | Row height strip → face; the rail grows; rows below descend. | `LayoutAnimation`, `update: { spring, springDamping: 0.85 }` on iOS — ≤2pt overshoot, one settle, never a second bounce. Android: `easeInEaseOut` (its `LayoutAnimation` spring is coarse; a stutter is worse than no spring). |
| 0 → 400 | Face body opacity 0 → 1. | `create: { easeInEaseOut, opacity }` |
| 0 → 250 (`durationMedium`) | The sentence translateY −4 → 0 — it settles into place a beat after the box does. | The one hand-rolled value on unfold; `Easing.out(Easing.cubic)`, native driver; **only on an owner-caused unfold** |
...
```ts
const FOLD_LAYOUT   = { duration: theme.durationMedium, create: { type: 'easeInEaseOut', property: 'opacity' }, update: { type: 'easeInEaseOut' } };
const UNFOLD_LAYOUT = { duration: theme.durationSlow,   create: { type: 'easeInEaseOut', property: 'opacity' },
                        update: Platform.OS === 'ios' ? { type: 'spring', springDamping: 0.85 } : { type: 'easeInEaseOut' } };
```
The Android enable flag is already at the top of `InsightCard`. The fold is two-phase: a `settling` state mounts an `Animated.View` around the body (mounted **only** while settling — the `ArrivalStage` rule, so nothing-folded stays byte-identical to the shipped tree), runs the 150ms parallel, and on `finished` calls `configureNext(FOLD_LAYOUT)` + `setState('folded')`. Unfold is one commit: `configureNext(UNFOLD_LAYOUT)` + `setState('face')`, the sentence's `Animated.Value` seeded at −4 only when the unfold is owner-caused. **Tests:** mock `LayoutAnimation`; the two configs fire on the two press paths only — never on a `cached` change, the reopened path, or under reduced motion; the settling wrapper is absent from the idle tree (snapshot).

**What makes it delightful, for the PM:** the coloured line never breaks. The same rail that stood beside the headline for six weeks is the line beside the one-line strip — the words leave, the card closes around the line in one breath, and everything below rises with it. Nothing is added and nothing is taken away; the owner watches the card get smaller while its thread stays. Unfolding is that line growing back, with the sentence settling in a beat after the box.

---

## 13. The card design — three directions for mock round 2 (DF-11 — PR 4, CUL-790; Designer, round 2)
