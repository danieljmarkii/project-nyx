...
| DF-10 | The fold motion ("design delight") | PM ask, 2026-09-03: *"as these cards are expanding and contracting, let's add some design delight."* Spec'd in **§12** from the Designer's round-2 pass: owner-caused fold/unfold only (an automatic re-open still lands before paint, FS-9), visual only (no haptic — `InsightCard` is a scanned safety component), S1 plainness survives on every class, reduced-motion frame defined, no loop. Ships as **PR 3**. | Spec'd — the PM picks the variant on mock round 2 |
| DF-11 | The card design refresh | PM ask, 2026-09-03: *"if there's anything we can do to improve the design of the cards, let's explore those options."* Three directions drawn side by side in mock round 2 (**§13**); the pick is a PM ruling; the build is **PR 4**, after v1's fold ships. | Drawn — PM pick pending |
