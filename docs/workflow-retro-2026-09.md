...
**F6 — a `CUL` RANGE in a PR title creates attachments on both endpoints, and they close those issues on merge.** Found live, on this session's own PR. #829's title read `… (CUL-919 … CUL-928)`; the integration parsed the first and last identifiers, created an attachment on each, and assigned both to the PM. Merging would have marked **CUL-919 and CUL-928 `Done` with neither fix made** — the CUL-660 shape (attached as related work, closed on merge), reached by a different route. The body's bare mentions of the other eight created nothing, exactly as CUL-803 predicts.

Both attachments were deleted and the range was removed from the title. Two consequences: **never put a `CUL` range in a PR title** — enumerate in the body, where a bare mention is inert; and the close predicate in CUL-926 must not treat "has an attachment" as proof of *intent*, because an attachment can now be shown to arise from a title string nobody meant as a commitment. Clause 1 needs the attachment **plus** a second signal (a closing keyword, or the issue already in a started state).
