...
| **D2** | Shared incident-analysis library | **Factor out the vomit-proven patterns into a shared module** (`supabase/functions/_shared/incident-analysis.ts`) reused by a refactored `analyze-vomit` and the new `analyze-stool`: the 3-layer escalation-floor pattern (system prompt → enum → deterministic floor), the image-fetch/downscale/base64-chunk utilities (5MB→3.93MB raw ceiling math, imgproxy 1568px downscale, magic-byte sniffing, chunked base64 encoder — all incident-agnostic today), and the `event_ai_analysis` upsert/re-analysis-preserves-edits write-back logic (Pattern 7). Per-type pieces (tool schema, system prompt content, contextual-flag computation, structured columns) stay local to each function. This is the skill's own predicted refactor point — deferring it to a third incident type would mean building `analyze-stool` as a second full copy-paste of an 863-line file, which the codebase's own "would a senior engineer at Linear be comfortable maintaining this" bar does not survive. **CONFIRMED 2026-07-16 (PM delegated the call to the team; team ratified) — build the shared module, shaped as a _descriptor-driven pipeline_, not a utility grab-bag.** The PM's steer was "make sure this scales" beyond the two current types (riff: a future coat-quality photo read), which reframes D2 from "dedupe 2 functions" to "what seam makes the Nth type cheap." Evidence it's overdue: three functions already copy-paste the gate + image-media logic by hand, each with an in-code "consolidate into `_shared/` later" TODO (`extract-food-from-photo`, `extract-medication-from-photo`, `analyze-vomit`). **Shape:** one `runIncidentAnalysis(descriptor, req)` owns the full pipeline (auth → confused-deputy guard → cap/flag gate → context SQL → image fetch/downscale → vision call → escalation floor → never-clobber write-back → response); each type is a small **descriptor** (enums, tool schema, system prompt, contextual-flag SQL, floor predicate, copy builders + the Track-2 `FUNCTION_KEY`/`FLAG_KEY`/`CAPS`). A new type = descriptor file + schema migration + one monetization config row — **no handler copy**. **Discipline (avoids over-abstraction):** extract only the seams vomit AND stool both exercise — no coat-specific extension points for a feature that doesn't exist; the descriptor shape is validated by two real, clinically-different types (vomit's flat consistency vs. stool's Bristol scale + melena/haematochezia split), which IS the "earn the right to abstract" threshold. Coat quality is the seam's **acceptance test** ("could a 3rd type be added touching no handler code?"), not code we write now. **Safety guardrails on the framework (non-negotiable):** the framework owns the escalation *mechanism* (contextual flags force `worth_a_call`, model can't downgrade, recommendation enum has no reassuring value) but each descriptor supplies its own per-type floor *rules* (stool's D5 ≠ vomit's), and **every new descriptor still triggers its own mandatory `adversarial-reviewer` pass + reassurance-word regex test — that DoD line is NOT inherited from vomit's prior review.** **Regression backstop (PR 2 AC):** `analyze-vomit`'s existing test suite passes UNMODIFIED after the refactor (zero behavior change). No runtime coupling — `scripts/deploy-edge.sh` esbuild-inlines the `_shared` import into each function's self-contained bundle, so functions still deploy independently; PR 2 lands the module + vomit refactor together so they can't drift mid-migration. |
