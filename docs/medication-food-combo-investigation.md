...
### Phase B — Slice C: the combo (GATED on G2–G3; G1 resolved — auto-dismiss + fail-safe)
...
- **PR B2 — Combo entry + linked-dose write** (host decided by G1 = the auto-dismissing card; **tests required**). The opt-in "+ Gave a med with this?" line on the treat/meal completion card → `MedicationPicker` → logs a dose carrying `paired_event_id` + `how_given` inferred from the food's type. Reuses/extends `insertMedicationDose`. The "Logged together" confirmation surface; show active-pet context (multi-pet catch). Reviewers: Designer + Jordan + Sam + QA. **AC:** the no-med path stays one tap; the combo links correctly; wrong-pet guarded.
- **PR B3 — Safety coupling (Flow B) — `adversarial-reviewer` MANDATORY.** When a linked vehicle is marked `refused`/`picked`, surface the owner-confirmed "did she still get it?" prompt **on the auto-dismissing card (G1)**; route to `partial`/`missed`/unconfirmed; **never auto-flip, never silent-given** (n=1-never-reassures). **The G1 fail-safe is the load-bearing rule:** an *unanswered* prompt leaves the dose **unconfirmed (never "given"), uncounted toward compliance**, and the open question **resurfaces calmly** (Home insight / History flag — build-time sub-choice). This is the medication analog of `analyze-vomit`'s escalation floor (`clinical-guardrails` Pattern 2: no path to a reassuring verdict by construction). Name the "ate-around-the-pill" residual (G4). Reviewers: `adversarial-reviewer` + `clinical-guardrails` + Dr. Chen + `nyx-voice`. **AC:** a not-finished vehicle never records a clean "given" — including when the card auto-dismisses unanswered; the reviewer states the counterexample it tried.
...
- **Gated:** Phase B on G2 + G3 (G1 resolved); Phase C on Phase B data + adversarial.
