...
**Version:** 2.3 — §3.3a: the rundown's second door (Get ready, CUL-903) | **Date:** 2026-09-11 | **Status:** **Build-ready — no PM blockers**
...
**Session record:** divergent/convergent product-team session 2026-07-18 (PR #394) — full persona pass, UX-landscape research (`docs/research/2026-07-ask-ai-ux-landscape.md`), PM rulings D1–D7, design-mock review (`docs/ask-mockups.html`, design-locked at PM review). **v2.1 (same day):** PM ratified D2 with an *expanded* boundary (notes + photos in scope — overriding the team's tier-2 recommendation; conflict recorded §6/§15) and deferred the D7 name workshop ("Ask" placeholder ships). **v2.2 (spec-review pass, same day):** the product team's review of the build-ready spec caught two conversation-grain blockers in the cap/lifetime model — the free credit collided with D6's provenance tap-through. PM ruled **D8** (the conversation survives in-app navigation, incl. the tap-through — great UX, keep it) and deferred **D9** to the team (the free credit commits on the first *substantive answer*, never on surface-open or a deflection/floor). See §2 D8/D9, §3.2, §5.3, §9.1, §10, §13 AC-15/16, §14 S7.
...
| D8 | Conversation lifetime | **The conversation survives in-app navigation.** It is held in **ephemeral in-memory client state** (Zustand) and survives every in-app navigation — **including a provenance tap-through to History/Patterns and back** (D6's core interaction). It ends only on app background/kill, an idle timeout (§14 S7), or the owner explicitly starting fresh — **never on a bare navigation-away.** No durable transcript is persisted (reconciles §10: in-memory ≠ stored). Fixes the review blocker where the most-encouraged interaction — verify the number at its source, then ask a follow-up — silently ended the conversation. | PM 2026-07-18 (v2.2) |
| D9 | Conversation-credit commit | **The free `ask_conversation` credit commits on the first *substantively-answered* question — never on surface-open or on a non-answer.** Free (no credit spent): the deflections (`clinical_judgment` / `unsupported` / `ambiguous`), `NotEnoughData` floors, and the cap/disabled/LLM-down fallbacks — they don't deliver the teaser's value, so they don't spend it. Commits the credit: a tool-plan answer, a relayed safety card, or a general-mode answer. **`ask_message` still counts every model call** (cost/abuse backstop intact) — the two grains cleanly split *value*-metering (conversations) from *cost*-metering (messages). | PM 2026-07-18 (v2.2) — PM deferred to the team |
...
- **Follow-ups:** in-session context (statelessness was Fitbit's documented failure). The conversation lives in **ephemeral in-memory client state** and **survives in-app navigation — including the provenance tap-through to History/Patterns and back** (D8); it is **not** ended by leaving the surface. It ends on app background/kill, an idle timeout (§14 S7), or an explicit new-conversation — with **no durable transcript persisted** (§10). The rundown is the pinnable artifact.
...
One tap assembles the clinician's opening answers: symptom counts w/ denominators, timing cluster, appetite rate, weight range over weigh-ins, current meds + last dose, since-last-visit changes. Built client-side from the existing aggregate layer (`lib/analytics.ts`, `lib/weight.ts`, med queries) — **works capped and offline**. Every tile taps through to its source. "Share the full vet report" hands off to the Step-9 flow. No adjectives, no verdicts — the report's register.

#### 3.3a The rundown's second door — "Get ready" (CUL-903 / vet-visits VV-5, 2026-09-11)
`/rundown` now has **two modes on one route**, and the vet-visit companion's spec (`docs/nyx-vet-visits-requirements.md` §4.1 B1) owns the second. With an **`appointmentId`** param the screen is **Get ready**: the same deterministic, offline, capped-safe rundown under a job title, reached from the Home appointment strip, the Pet-tab card, the visits list and Ask (Ask keeps its chips, unchanged). Without the param it is this section's screen, unchanged.

- **The rundown block is byte-identical between the modes** — asserted, not intended: both render `components/ask/RundownBlock.tsx` and `app/rundown.getready.test.tsx` deep-equals the two rendered subtrees. "Promote, don't rebuild" (the PM) is a test here.
- **Get ready makes zero model calls**, like this section's screen — and the near-miss is worth recording because it is not obvious: `useSignal()` refreshes a stale cache (`readSignalsAndRefresh` → `regenerateSignal` → `functions.invoke('generate-signal')` → the Haiku phrasing call), so the natural way to read findings on this page would have spent one. It reads `readSignalCache` **directly** instead. A spy over `functions.invoke` holds it through mount and every tap.
- **"Worth raising"** sits above the rundown block: a capped, deterministic list where every row **quotes** a sentence the app already states — the Signal's own phrased sentence, the trial strip's header and line, the rundown's past-course row in its H1 register, the rundown's weight tile. One source per row, **never a new count with a new window** (G6; the CUL-746 failure mode). Safety findings lead and sit above the cap; a quiet record renders **no section at all**, never a row saying "nothing to raise".
- **"Your questions"** — the owner's own, stored on `vet_appointments.questions`, carried into the visit as ticks (VV-4), **never read by a model in v1**.
- **The share moves (R-share).** The PM doubted the rundown should go to the vet and all nine persona interviews agreed independently: Get ready's single primary is **Send the vet report**, and this section's "Share the rundown" is demoted to ⋯ → **Copy as text**, addressed to no one — "for whoever's taking {pet} in". That is CUL-206's gap answered: the label now says what it does rather than promising a persistence the app does not have. The plain-rundown mode keeps its own two buttons unchanged.
...
- **`ask_conversation`** — the **monthly** free-tier *value* grain (`month_count` from `record_ai_usage`). **Committed on the first substantively-answered question, NOT on surface-open (D9):** deflections (`clinical_judgment`/`unsupported`/`ambiguous`), `NotEnoughData` floors, and the cap/disabled/LLM-down fallbacks are free; a tool-plan answer, a relayed safety card, or a general-mode answer commits it. A conversation that survives a tap-through (D8) stays **one** conversation.
- **`ask_message`** — per model call; the **cost/abuse** grain — daily backstop + per-conversation bound (~10) enforced in-function. **Counts every model call regardless of D9** (a deflection is free on the conversation grain but still cost-metered here), keeping value-metering and cost-metering cleanly separate.
...
| **A4** | The `ask` Edge Function (§5): ownership gate → flag/cap checks → Sonnet tool-loop → `validateAnswer` → deflection templates → typed 200s; general-mode path (flag-gated). **Commit `ask_conversation` per D9 — on a substantive answer only, never on a deflection/floor/fallback; `ask_message` on every model call** (AC-16). Cached-read relay included; **live photo reads deferred to A8** (ship the honest "no read yet — open the event to run one" state). Golden + adversarial deno fixtures (§13, incl. injection-via-note). Deploy (new fn, `verify_jwt=true` — safe pre-client). Secrets Register: add `ask` to the `ANTHROPIC_API_KEY` row. | **adversarial-reviewer MANDATORY**; **rls-privacy-reviewer MANDATORY**; code-reviewer |
| **A5** | Client surface: `app/ask.tsx` states (§3.2) + `components/ask/*` (typed component-descriptor renderer over existing pips/Sparkline/ranked rows), Home header pill (D5), suggested-chips generator, cap/disabled/offline/empty states, meter, the §6.5 policy line. **In-memory conversation store (Zustand) that survives navigation + the provenance tap-through (D8) and ends only on background/idle/explicit-new** (AC-15). | code-reviewer; nyx-voice; Designer; Jordan+Sam; **pm-feature-review**; QA state matrix |
...
15. **Conversation survives navigation (D8):** a provenance tap-through to History/Patterns and back does **not** start a new conversation or burn an `ask_conversation` credit; the in-session context is intact on return.
16. **Credit commits on answer, not on non-answer (D9):** a deflection / `unsupported` / `ambiguous` / `NotEnoughData` floor does **not** increment `ask_conversation`; a substantive answer (tool-plan / relayed-safety / general-mode) does; **every** model call increments `ask_message` regardless.
...
S1 streaming vs single-shot (lean: single-shot + skeleton) · S2 in-session context depth (lean: last ~6 turns) · S3 model id via config (lean: yes) · S4 server shared-util placement for the flag resolver — **largely answered by main's precedent**: `supabase/functions/_shared/` exists since #390 and `scripts/deploy-edge.sh` bundles it (verify at A2) · S5 rundown entry placement when `ask_enabled` is off (lean: Ask-internal v1; revisit with B-372) · S6 whether A8's live read auto-runs or asks first ("want me to look at the photo?" — cost + surprise; lean: one-tap confirm chip) · S7 conversation end-triggers (D8) — the concrete idle-timeout and whether app-background alone ends it (lean: ends on app background OR ~30-min idle; provisional, tune with dogfood).
...
**v2.2 amendments (2026-07-18 spec-review pass):** **D8** (the conversation survives in-app navigation, incl. D6's provenance tap-through — no longer ended by leaving the surface) + **D9** (the free `ask_conversation` credit commits on the first *substantive* answer, never on surface-open or a deflection/floor; `ask_message` still cost-meters every model call). QA/Designer/Jordan/Sam findings from the review of the build-ready spec — the cap/lifetime model collided with the answer-card's own tap-through. PM ruled D8 explicitly and deferred D9 to the team. No architecture change; the fixes live in the client conversation store (D8, A5) and the Edge Function's cap-commit ordering (D9, A4).
