...
- **Live photo reads (A8)** additionally consume the **existing `analyze-vomit` counter** (10/day, ratified) — one cap governs per-incident reads product-wide, however they're triggered.
...
| **A8** | **Live photo reads inside Ask (§6.2/§7.7)** — the tool that invokes the `analyze-vomit` machinery run-or-read-cache (transform-only fetch, same cap counter, result persisted to `event_ai_analysis`), + the client read states. After A5. | **adversarial-reviewer MANDATORY**; **rls-privacy-reviewer MANDATORY**; clinical-guardrails; Dr. Chen |
...
> **A8:** Read `docs/nyx-ask-requirements.md` (§6.2, §7.7, §9.1, §12 A8). Wire live photo reads into the ask tool-loop by reusing the `analyze-vomit` machinery run-or-read-cache: transform-only photo fetch (the PR 7 EXIF/GPS-stripping pattern), the shared 10/day read counter, result persisted to `event_ai_analysis`, owner-edited fields authoritative. Build the client read states. clinical-guardrails governs; adversarial-reviewer and rls-privacy-reviewer are mandatory; extend the eval set with the §7.7 falsification fixtures.
...
S1 streaming vs single-shot (lean: single-shot + skeleton) · S2 in-session context depth (lean: last ~6 turns) · S3 model id via config (lean: yes) · S4 server shared-util placement for the flag resolver (esbuild bundling constraint) · S5 rundown entry placement when `ask_enabled` is off (lean: Ask-internal v1; revisit with B-359) · S6 whether A8's live read auto-runs or asks first ("want me to look at the photo?" — cost + surprise; lean: one-tap confirm chip).
