...
**Version:** 1.1 — RATIFIED + mock round 1 rulings folded in (PM, 2026-08-24; see D8/D9 and D4/D10–D12) | **Last Updated:** 2026-08-24 | **Status:** SCOPING ONLY — no build committed (D5 stands). Mock round 1 held 2026-08-24 (D4 ruled, R3 adopted, Q4 confirmed); round 2 explores the full-spine navigability mechanism (Q7) and the picker fixes. Every build wave below remains a proposal awaiting its own greenlight — and W1 now carries the log_picker_v2 pre-GA gate (D12).
...
| Q7 | **The full-spine navigability mechanism** (PM-raised at mock round 1: "is the full spine too much?"). Two candidate mechanisms, both rendered in mock round 2: **(a) configurable tracked events** — a Settings surface toggling leaf visibility per pet (hide ≠ delete: hidden leaves stay reachable via a "Show more types" reveal, rows/detection/read surfaces never change, safety leaves default-on) vs **(b) the "frequent for this pet" row** (§6's bounded row). The PM leans toward (a) accomplishing (b)'s job; scope question inside (a): account-level vs per-pet (the grid is already per-pet via species — per-pet composes; a multi-pet household's chronic cat ≠ its healthy dog). Sub-question for Dr. Chen at build: may safety leaves be hidden at all? Decide no later than the first wave that pushes a species grid past the fold (W3–W4); W1–W3 density needs neither. | PM + Designer + personas, mock round 2 |
