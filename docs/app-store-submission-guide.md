...
| 4 | Pick + configure the production SMTP provider | PM | B-152 (part 1) | 🟡 In progress 2026-07-24 — **Resend** chosen; `getculprit.app` **verified** as a sending domain (DKIM + SPF MX + SPF TXT all verified via the Resend API; DMARC `p=none` added as a bonus record). Supabase SMTP configured (`smtp.resend.com:465`, sender `support@getculprit.app`, name `Culprit`). **Residual: no send has been observed yet** — one confirmation email must appear in Resend's logs before this closes |
...
| 9 | Flip email confirmation ON + verify the signup path | Mixed | B-152 (part 2) | 🟡 Claude half ✅ 2026-07-24 (#436) — B-401 dead-end fixed + all three raw-auth-error paths mapped (`lib/authErrors.ts`). PM half open: confirmation toggled ON in the dashboard, but **the device pass + the two RLS dashboard checks are not done**, and step 4's first observed send is a hard prerequisite. New finding → **B-432** (signup confirmation sends no `emailRedirectTo` / no post-confirmation landing) |
