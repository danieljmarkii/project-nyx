...
| 12 | Screenshots | PM | B-269 | 🔵 Plan + design direction ratified 2026-08-09 — `docs/store-screenshot-plan.md` §0.5 (D-SS1 = direction C "night opens, day carries"; D-SS2 = the **designed set is the submission set**, plain captures the fallback floor; D-SS3 = report at frame 3). Evidence: `docs/research/2026-08-app-store-creative-landscape.md`. Round-3 composed set is in the mock (full C template + search-card preview + the frame-6 A/B call). Remaining: the joint caption/keyword pass (with step 13), the frame-6 call, the 1320×2868 export pipeline, then PM capture — gated on step 11 (demo seed) |
