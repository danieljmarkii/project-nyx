...
| 12 | Screenshots | PM | B-269 | 🔵 Plan + design direction ratified 2026-08-09 — `docs/store-screenshot-plan.md` §0.5 (D-SS1 = direction C "night opens, day carries"; D-SS2 = the **designed set is the submission set**, plain captures the fallback floor; D-SS3 = report at frame 3). Evidence: `docs/research/2026-08-app-store-creative-landscape.md`. **R3-1: the bezel template was sent back** ("still app on a background") — copy/order/posture stand; mock round 4 renders three bezel-free concepts (evidence-blown-up / constellation / specimen). Remaining: **D-SS4** (concept pick), round-5 full set, the joint caption/keyword pass (with step 13), the frame-6 call, the export pipeline, then PM capture — gated on step 11 (demo seed) |
