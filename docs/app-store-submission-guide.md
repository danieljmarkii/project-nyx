...
> **Live tracking moved to Linear (2026-08-20):** the **App Store Launch** project (team Culprit — https://linear.app/projectnyx/project/app-store-launch-57bc8d011b9d) now owns per-task status, kickoff prompts, and the D1–D11 decisions; every row below maps to a `CUL-` issue there. This table stays as the historical runway + the how/why reference — don't maintain status in both places (the B-002/CUL-265 lesson: one launch checklist, not two).
...
| 4 | Pick + configure the production SMTP provider | PM | B-152 (part 1) | ✅ **Done 2026-07-25 — verified by a live send, not by configuration.** Resend; `getculprit.app` verified as a sending domain (DKIM + SPF MX + SPF TXT; DMARC `p=none` bonus). Supabase SMTP = `smtp.resend.com:465`, user `resend`, sender `support@getculprit.app` / `Culprit`. Proof: `POST /auth/v1/signup` → **HTTP 200 with `confirmation_sent_at` and no `access_token`** (so confirmation is genuinely ON) → email delivered to the PM's inbox. Secrets Register row added. |
...
| 11 | Seed the App Review demo account + reviewer notes | Mixed | B-271 | 🟡 **Code complete (2026-08-11):** spec v2.1 applied; PR 1 seed shipped #627; PR 2 reviewer notes (`docs/app-review-notes.md`) shipped #628; demo account `support@getculprit.app` + Cooper **created** (runbook step 1). Remaining = the PM-gated live seed, runbook §12.3 steps 2–7 → **CUL-188** (blocked by the CUL-557 deploy-currency pass) |
| 12 | Screenshots | PM | B-269 | 🔵 Plan + design direction ratified 2026-08-09 — `docs/store-screenshot-plan.md` §0.5 (D-SS1 = direction C "night opens, day carries"; D-SS2 = the **designed set is the submission set**, plain captures the fallback floor; D-SS3 = report at frame 3). Evidence: `docs/research/2026-08-app-store-creative-landscape.md`. **D-SS4 ruled:** the round-3 bezel template stands, **enriched** (mock round 5): dot-lane receipt on the hero's insight card (needs `signal_design_v2` on the demo account), by-day chart in the report frame (→ **B-742**, a `generate-report` change riding the B-494-held redeploy), species-matched illustrated avatar (uploaded as Cooper's pet photo — no product change). Frame 6 ruled **6A (full Home)** — **all design decisions closed 2026-08-09**. **Export pipeline BUILT 2026-08-09 (#625):** `scripts/store-screenshots/` — the round-5 template at true 1320×2868 with the real Geist/Newsreader faces + a zero-dep headless-Chromium renderer; `--draft` renders all six frames from stand-in screens today (six draft PNGs delivered for review); capture mode composites the §3 captures into frames 2–6 and builds the frame-1 hero by cropping the Signal cards out of the Home capture (`captures/hero-crops.json`); every output verified 1320×2868 RGB no-alpha. Remaining is production only: the joint caption/keyword pass (with step 13 — edits only the template's `FRAMES` strings), B-742 + `signal_design_v2` presentability for the enriched frames, then PM capture — gated on step 11 (demo seed) |
| 13 | Listing copy + age rating | Mixed | B-269 | 🟡 **Age rating ✅ Done 2026-08-09 — entered in ASC, rated 4+** (every content descriptor None, every capability No, Kids Category off; recorded in `docs/app-store-age-rating.md`; consistent with the 1.4.1 description + B-270 disclaimer). **Copy half still open:** drafted 2026-08-09 → `docs/store-listing-copy.md` (subtitle, promotional text, description, keywords — all char-verified in-limit; clinically honest per 1.4.1). **Awaiting PM review + one decision:** keep the ASC-locked subtitle "Track symptoms, find triggers" vs. adopt "Diet trials & symptom diary" (recommended, carries the diet-trial wedge). |
...
**Age rating (PM, in ASC): ✅ Done 2026-08-09 — rated 4+.** The 2025-revised questionnaire (tiers 4+/9+/13+/16+/18+) was completed with every content descriptor **None**, every capability question **No** (incl. unrestricted web access — the WebViews render only the app's own report/PDF content, not a general browser), and the Kids Category **off**. The **medical/treatment information** question was answered *the app provides informational pet-health tracking, not medical treatment or diagnosis* (consistent with B-270's disclaimer and the description). Full answer record: `docs/app-store-age-rating.md`.

**Confirm with:** `Guide step 13 complete: copy entered, age rating <tier>.` _(Age-rating half already done — 4+, 2026-08-09; this line closes when the listing copy is entered too.)_
