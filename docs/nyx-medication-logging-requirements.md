...
- **A zero-row local UPDATE throws.** Moving the write did not by itself cure the silence above — it RELOCATED it, from PostgREST's zero rows to SQLite's, and the first cut of VV-3 shipped exactly that (both mandatory reviews found it independently). `UPDATE … WHERE id = ?` against a row the mirror does not hold returns `{ changes: 0 }` and resolves. It is reachable while the Pet-tab card still reads the server (below): two phones on one household account is enough, and the consequence is an owner who believes she ended a steroid course while `generate-report`, `ask` and `generate-signal` all go on reading `status = 'active'`. `updateRegimen` and `endRegimen` therefore throw on `changes === 0`, before the hydration tick and before the flush — the same guard, for the same stated reason, that all four of `lib/db.ts`'s by-id local updates already carry.
...
- **Known gap, not introduced here:** the Pet tab's own card reads regimens from Supabase (`app/(tabs)/profile.tsx`), unlike the med strip, widget, rundown and dose-linking, which read the local mirror. Both loaders re-run on every tab focus and overwrite state unconditionally on a SUCCESSFUL read, so the gap is not only a cold-launch one: any focus cycle whose remote read lands before the queued push erases an already-rendered course, and an ended course reappears as active. It is also what makes the zero-row case above reachable at all. Tracked as CUL-938 — fixing that read closes both.
