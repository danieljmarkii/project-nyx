...
8. **The actual submission critical path is unchanged by this session:** B-039 (in-app account deletion, 5.1.1(v) hard blocker — **DONE, verified 2026-07-16**), B-229/230 (legal docs — in progress), B-267 (permission strings), B-268 (privacy label), B-269 (listing assets), B-270 (disclaimer), B-271 (demo account), B-272 (store record/name), B-273 (support URL/site — in progress), B-280 (password reset). **Monetization is not on this path. Do not let it block submission.**
