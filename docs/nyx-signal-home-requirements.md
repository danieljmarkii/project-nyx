...
**RETIREMENT RECORD — FR-FLAG-5 exercised (GA 2026-08-20).** The PM called GA for both Signal betas on 2026-08-20 (execution plan: CUL-546). `signal_design_v2` graduated to generally available and was retired through the removal PRs, in the load-bearing order the two-gate architecture forces: **(1)** the client flag-off render path was deleted — the surface renders unconditionally-when-present (GA-1, CUL-547, shipped via #690); **(2)** the `app_config` row was flipped to `{"enabled": true}` for old-build continuity during rollout (Phase 2, CUL-549, no PR); **(3)** the `app_config` row was deleted (GA-4, CUL-551, migration `060_retire_signal_beta_flags.sql`). Nothing in the product references `signal_design_v2` any longer. The beta **mechanism** is untouched (`resolveAllowlistFlag`/`useAllowlistFlag`, the opt-in store, `BETA_REGISTRY`) — `widget_enabled` and `log_picker_v2` still ride it; this removed one registry row + one key, never the primitive. This is one of the **first two graduations** exercised through the B-712 D7 registry policy (the other is `signals_v2`; see `docs/nyx-beta-features-requirements.md` §4.3.1) — the `reviewBy` (2026-11-08 region) closed early as *graduate* by an explicit PM call, exactly as the policy intends.
