...
- **Linear (team Culprit)** — the monetization/analytics items behind the model: the freemium-gate reconciliation (legacy B-263 — now *unblocked* by D-M1, execution still open), the monetization build track (legacy B-329–B-333: flags, mock flag-off, RevenueCat, manual protein capture, care-first messaging), and retention + analytics instrumentation (legacy B-047/B-016 — the model's future ground truth). Find each by its legacy `B-NNN` (every migrated issue keeps a `Legacy` label + a `_Migrated from docs/backlog.md (B-NNN)_` footer); `docs/backlog.md` itself is frozen.
