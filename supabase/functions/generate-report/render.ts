...
/**
 * WHY an off-diet row is here — ONE rule, two consumers (CUL-746).
 *
 * Appendix C's "Why it's here" column has stated the precedence since round 4:
 * DATED MEMBERSHIP OUTRANKS THE RUNG, because when both are true the date is the
 * reason the row is here and the rung is the misleading half. Page 1 never got it.
 * It printed the rung breakdown and appended the date as a trailing "also", so the
 * SAME seven feedings were counted twice under two reasons and the misleading one
 * led, in bold, on the page a vet actually reads:
 *
 *   "Of those 7: 7 were not recognised as trial food (no ingredient list captured,
 *    so nothing more can be said about them). 7 were also fed before that food was
 *    permitted."
 *
 * …four lines under a line naming that same food as the trial diet, and three pages
 * above an appendix that got it right. Cold read 16, on a hydrolysed elimination
 * trial started Apr 21 whose `diet_trial_foods` row records the primary diet as
 * allowed from Jun 1: page 1 told the vet 7 of 8 feedings missed the trial diet —
 * 88% non-compliance — when all seven WERE the prescribed food. Dr. Chen: "the truth
 * is the opposite: this is the best evidence on either page that the owner was
 * actually feeding the prescribed food." C6 makes that the one thing on this report
 * that judges a person, so an additive framing is not a copy nit here.
 *
 * The rule lives in one function rather than in two places that agree today: the
 * appendix's copy of it was already correct while page 1's was absent, which is what
 * a duplicated rule looks like from inside.
 */
type ExposureReason = 'fed_before_permitted' | 'derived_protein' | 'unrecognised'

function exposureReasonOf(row: { rung: string | null; permittedLaterFrom: string | null }): ExposureReason | null {
  // Truthy, not `!== null`, so this is exactly the test the Why column has always
  // applied — and so the `!` its caller keeps is sound rather than merely quiet.
  if (row.permittedLaterFrom) return 'fed_before_permitted'
  if (row.rung === 'derived_protein' || row.rung === 'unrecognised') return row.rung
  return null
}
 ...
 * "9 / 10 · Feedings not matched to the trial diet" off the tile and wrote down
 * "restart the trial, blame the owner" before reaching the line below it that
 * exonerates the owner — at a third of the type size. So both surfaces take the same
 * numbers from the same function, and an edit to one cannot silently leave the other
 * saying something else.
 *
 * The three counts always partition `offDiet` exactly — the number the tile and the
 * sentence both name — so neither can render a breakdown that fails to add up to its
 * own denominator.
 */
interface ExposureBreakdown {
  /** Rows whose reason is the DATE, not the rung. */
  earlyCount: number
  proteinCount: number
...
/**
 * Does a permission date on this report fall outside the evidence window's year?
 * Decided ONCE for every place such a date renders — the page-1 clause and appendix C's
 * Why column and Dates column — never per site (CUL-69).
 *
 * `fmtDay` prints "Mon D" with no year, which was safe here only by a construction this
 * field does not have: an `allowed_from` is bounded by the TRIAL, and on an un-ended
 * trial (B-422's steady state) that is unbounded forward while the report window can be
 * a hand-picked ten days years earlier. The adversarial pass ran it — a trial started
 * Nov 2025, window Nov 1–20 2025, a chew permitted from Mar 2026 — and page 1 printed
 * "(allowed from Mar 15)" beside "Nov 1 – Nov 20, 2025". A reader resolves a bare date
 * to the nearest year, i.e. March 2025, eight months BEFORE the feedings, which flatly
 * contradicts the clause it sits inside.
 *
 * Per band rather than per date, because CUL-69's second half is that a CONDITIONAL year
 * is worse than none when two dates share a sentence or a row: a bare date beside a
 * year-stamped one inherits that year. When this is false the output is byte-identical
 * to what shipped before.
 */
function permissionDatesNeedYear(t: NonNullable<ReportSnapshot['trial']>): boolean {
  const yearOfDay = (d: string | null): number | null => dayParts(d ?? '')?.y ?? null
...
function exposureBreakdown(t: NonNullable<ReportSnapshot['trial']>): ExposureBreakdown {
  const { offDiet, byRung } = t.exposures
  // The items array is the adapter's second pass over the SAME predicate and the same
  // evidence range, and is what appendix C's rows are joined from — so page 1, the tile
  // and the appendix all partition the identical population.
  const early = t.exposures.items.filter(
    (x) =>
      exposureReasonOf({ rung: x.classification.rung, permittedLaterFrom: x.permittedLaterFrom }) ===
      'fed_before_permitted',
  )
  // Rows are MOVED out of their rung, never re-counted: `byRung` is the module's own
  // tally and sums to `offDiet` exactly (only rungs 2 and 3 ever set `offDiet`), so
  // debiting it cannot produce a breakdown that fails to add up. Each `Math.min` caps
  // the debit at the rung it is taking from, because a snapshot whose `items` disagreed
  // with its own `byRung` must still render a partition that sums — and a negative
  // count is the one thing worse than the additive framing this replaces.
  const movedProtein = Math.min(
    early.filter((x) => x.classification.rung === 'derived_protein').length,
    byRung.derived_protein,
  )
  const movedUnrecognised = Math.min(
    early.filter((x) => x.classification.rung === 'unrecognised').length,
    byRung.unrecognised,
  )
  const earlyCount = Math.min(movedProtein + movedUnrecognised, offDiet)
  // EACH CLAUSE TAKES WHAT IS LEFT. Identical to `byRung.x - moved.x` whenever the
  // snapshot is self-consistent; on one that is not, this is what stops the breakdown
...
function exposureSentences(t: NonNullable<ReportSnapshot['trial']>): string[] {
  ...
  if (offDiet > 0) {
    // ── PRECEDENCE, NOT ADDITION (CUL-746) — see `exposureBreakdown` ───────────
    //
    // Round 5 put the dated-membership fact on this page as a trailing "also",
    // which fixed the cross-check (4 against 3) and left the misleading half
    // leading. Round 16 executed the cost on the report that shape was built for:
    // seven feedings of the PRESCRIBED diet read as "not recognised as trial food",
    ...
    if (proteinCount > 0) {
      parts.push(`${num(proteinCount)} carried a protein the trial diet does not`)
    }
...
function atAGlance(snap: ReportSnapshot): string {
  ...
  // …AND "THE TRIAL'S OWN RANGE" IS NOT ONE RANGE (CUL-746). Coverage prints the trial's
  // COVERAGE range (clipped at both ends) and off-diet prints its EVIDENCE range; those
  // differ by exactly the clips, and CLAUDE.md's rule is that the two must never be
  // conflated. While the off-diet tile printed no dates the aside was merely vague; now
  // that both tiles name their spans, a reader can see them disagree under a heading
  // asserting they are the same — the B-532/HR-7 one-page-two-renders class. Each tile
  // already states what it counts over, which was B-503's design; the heading only has
  // to say WHICH tiles depart from the window, not re-state a span for them.
  //
  // AND IT SAYS SO NEGATIVELY, because a positive claim is false on four of the
  // off-diet tile's six branches (no allowed list, an off-list bowl, unobserved
  // free-feeding, and the below-floor fallback) — those render a WORD rather than a
...
//
// THREE RULES GOVERN EVERY STRING BELOW, and each of them was a defect first:
//
//  1. **The unit is the answered DAY and the verb is the owner's ACT.** Never
//     "episodes" (§10.2a), never looks (R9). The denominator names what SHE did —
//     *answered on 42 of 46 days* — because a denominator that named her pet's state
//     would make a record-keeping number read as a health number.
//  2. **The absence is her CLAIM, never the report's observation.** *Marked nothing
//     unusual on 34 of the 42* — `marked`, with its denominator, every time. The
//     round-2 adversarial pass (G9) caught the first draft stating it as a finding.
//  3. **The absence is never a bar.** A bar invites a length comparison, and on the
//     first draft the LONGEST bar was the reassuring one (81% of the track). Floor 3:
//     coverage is never a good day. Only symptom-class counts are ever sized here.
//
// And one that is easy to lose in a refactor: ACTIVITY WORDS NEVER APPEAR ON PAGE 1.
// A rise in *Lively* never reassures, and a page-1 tally of activities invites exactly
// that reading. They are ink in the appendix, where the record is read row by row.
...
/**
 * The pre-first-day coverage clause — what stops *off on 6* reading as a new problem.
 *
 * Two forms, and the difference is whether the pull can stand behind a start date.
 * Complete ⇒ *(first Sep 2; answered on 118 days before it since May 3)*. Truncated ⇒
 * the count as a FLOOR with no date, because the earliest row is then a fact about the
...
/**
 * The intake sentence beside the absence claim (§5 honesty rule 12).
 *
 * The report NEVER withholds — it says the disagreement. Home keeps quiet days off the
...
/**
 * The page-1 line (§3 item 4).
 *
 * One sentence of counts, then the reconciliation clause, then intake. The clause is not
 * optional garnish: 6 + 4 + 2 + 34 is 46 over a denominator of 42, which is correct under
 * multi-select and reads as an arithmetic error to a cold reader. It is also stated in
 * BOTH directions — see `activityOnlyDays`.
 */
function noticedLine(snap: ReportSnapshot): string {
  const n = snap.noticed
  if (!n) return ''
  const parts: string[] = [`Answered on ${num(n.answeredDays)} of ${num(n.windowDays)} days`]
  for (const w of n.concernWords) {
    parts.push(`${noticedWordPhrase(w.label, w.dayCount)}${noticedFirstClause(w)}`)
  }
  if (n.absenceDays > 0) {
...
/**
 * The graph: bars for the symptom-class words, then the dated strip.
 *
 * Below the floor the bars do not render AT ALL — not a shorter bar, not a caveated one.
 * Four answered days and two *Off* draws a bar at half the track on a clinical document,
 * from a self-selected sample of four; the line and the strip carry the same facts with
 * their denominators visible, so nothing is lost but the misreading.
 */
...
function noticedBars(snap: ReportSnapshot, n: NoticedBlock): string {
  if (!n.barsRender || n.concernWords.length === 0) return ''
  const rows = n.concernWords
    .map((w) => {
      // The fill is days-with-the-word over days ANSWERED — never over the window, which
      // would silently reward a sparse record with a shorter bar.
      const pct = n.answeredDays > 0 ? Math.min(100, (w.dayCount / n.answeredDays) * 100) : 0
      return `
        <div class="nb-row">
          <div class="nb-lab">${h(w.label)}<span class="nb-first">first ${h(fmtDay(w.firstDay))}</span></div>
          <div class="nb-track"><div class="nb-fill" style="width:${pct.toFixed(1)}%"></div></div>
          <div class="nb-val num">${h(w.dayCount)} of ${h(n.answeredDays)}</div>
        </div>`
    })
    .join('')
...
/**
 * The dated 28-day strip.
 *
 * Its span is its OWN and is named in its own title, because it is not the bars' span —
 * the third adversarial pass found a reader meeting *6 of 42* above three ● here with no
 * way to tell they were one word over two windows. The per-word sentence beneath is the
 * reconciliation.
 *
 * An unanswered day draws a FAINT DOT rather than a blank. A blank is the visual default
 * and reads as "nothing seen", so a vomit's ▲ would float over a void.
 */
function noticedStrip(snap: ReportSnapshot, n: NoticedBlock): string {
  const s = n.strip
  ...
  // BOTH sides of this count are answered days (§6.11's lesson, applied to a caption):
  // a vomit day nobody answered is not a day nothing was seen, and saying so is what
...
function trialExposureTile(t: NonNullable<ReportSnapshot['trial']>): string {
  ...
  if (t.exposures.offDiet > 0) {
    // THIS BRANCH NAMES ITS SPAN, AND ITS REASON (CUL-746). Two repairs, one cause:
    // every previous fix landed on the branch a reader is least likely to act on.
    //
    // The span: `mayStateRecordClean` below was repaired for exactly this — the count
...
/**
 * CUL-875 — does the Noticed appendix render, and at which letter?
 *
 * THE LETTER IS COMPUTED, NOT `G`. The spec (§8) names it "Appendix G" on the assumption
 * that A–F always render; they do not. E is the meals appendix and is conditional, the
 * photos appendix takes E or F depending on whether meals rendered, and the "How to read"
 * page is deliberately unlettered. So on a report with no meals and no photos, Noticed is
 * **E**, and printing "G" would put a letter on the page that the letterhead's own
 * *Appendices A–D* line contradicts.
 *
 * That is not a hypothetical: the first round-2 artifact said "A–F" over a document whose
 * last appendix was D, and the cold read found a careful vet hunting for an appendix that
 * was not there — on a document whose entire pitch is "every figure traces to an
 * appendix". One source of truth for the letter, exactly as the photos appendix already
 * has. Noticed sits LAST because it is the owner's own layer beside the record, read after
 * the record, and because appending keeps every existing letter where a reader of an
 * earlier report expects it.
 */
function hasNoticed(snap: ReportSnapshot): boolean {
  return snap.noticed !== null && snap.noticed.days.length > 0
}
function noticedAppendixLetter(snap: ReportSnapshot): string {
  if (hasIncidentPhotos(snap)) return mealsAppendixVisible(snap) ? 'G' : 'F'
  return mealsAppendixVisible(snap) ? 'F' : 'E'
}
/** The last LETTERED appendix (drives the page-1 orient line + the divider): Noticed → photos → meals → D. */
function lastAppendixLetter(snap: ReportSnapshot): string {
  if (hasNoticed(snap)) return noticedAppendixLetter(snap)
...
function appendixDivider(snap: ReportSnapshot): string {
  ...
  const noticedBit = hasNoticed(snap) ? ` &middot; ${noticedAppendixLetter(snap)} — Noticed (owner's observations)` : ''
  ...
  <div class="divider">
    ...
    The appendices are the reference record behind every figure on page&nbsp;1: A — event log &middot; B — diet history &middot; C — off-diet exposures${medHistBit} &middot; D — medications${eBit}${photoBit}${noticedBit} &middot; How to read this report.
  </div>`
}
...
/**
 * How much of one note this document prints before it says it shortened it.
 *
 * `looks.notes` has NO length bound at rest (N-1's privacy review stored a 2 MB note),
 * and the service role reads whatever is there — so a cap belongs to the render or the
 * PDF is unbounded. The client's field caps at 300, so a normally-written note is never
 * touched by this; it exists for the row the client did not write.
 *
...
function noticedNoteHtml(note: string, petName: string): string {
  if (note.length <= NOTICED_NOTE_CAP) return `<div class="ng-note">&ldquo;${h(note)}&rdquo;</div>`
  return (
    `<div class="ng-note">&ldquo;${h(note.slice(0, NOTICED_NOTE_CAP))}&rdquo; ` +
    `<span class="ng-trunc">[shortened for this report &mdash; the whole note is in ${h(petName)}&rsquo;s record]</span></div>`
  )
}

/**
 * Appendix — Noticed (spec §8).
 *
 * GROUPED BY DAY, and the grouping is a clinical claim, not a layout preference: "units
 * are answered days, never episodes" binds the appendix's SHAPE as well as its counts, so
 * two looks on one day are two lines under ONE day heading and can never be counted as
 * two episodes by a vet scanning rows.
 *
 * Never summarised, never averaged, never coloured. Un-answered days are simply absent —
 * round 1's "didn't look" glyph left with its chip (R8), and the strip on page 1 is where
 * the un-answered days are visible as days.
 *
 * WHAT IS NOT HERE, BY CONSTRUCTION: a soft-deleted look (dropped at the query, N-1 rule
 * 2 — a note the owner took back never prints), a word outside the closed vocabulary
 * (dropped in assembly, rule 1 — the column is unbounded at rest and the review stored
 * `<script>…` in one), and the note itself whenever this is not the owner's own render
 * (§9 rule 4 — an unauthenticated mint carries no notes decision to inherit).
 */
function noticedAppendix(snap: ReportSnapshot): string {
  const n = snap.noticed
  ...
  const rows = n.days
    .map((d) => {
      const lines = d.entries
        .map((e) => {
          const words =
            e.words.length > 0
              ? e.words.map((w) => h(w.label)).join(', ')
              : e.outcome === 'nothing_unusual'
                ? `<span class="ng-abs">Nothing unusual</span>`
                : `<span class="ng-abs">&mdash;</span>`
          // Present-only, like every other disclosure on this document: a row that lost
          // nothing says nothing.
          const dropped =
            e.unknownWords > 0
              ? ` <span class="ng-trunc">[${h(e.unknownWords)} word${
                  e.unknownWords === 1 ? '' : 's'
                } this version of the app does not recognise, not shown]</span>`
              : ''
          const note = e.note ? noticedNoteHtml(e.note, petName) : ''
          return `
          <tr>
    ...
    .join('')
  // The note column's STATE is always said. A column that is simply missing reads as
  // "she wrote nothing"; a withheld one has to say it was withheld, or the report quietly
...
<section class="page">
  ...
  <p class="appx-sub">A once-a-day check the owner answers from a fixed list of words, grouped by the day she answered. Two entries on one day are two lines under that day &mdash; they are not two episodes. These are the owner&rsquo;s observations of ${h(
    petName,
...
interface ConfounderRow {
   ...
   *  permission, which outranks whichever rung also fired. Joins the group key
...
function groupConfounders(conf: ConfounderExposure[]): ConfounderRow[] {
  ...
  for (const c of conf) {
    ...
    // …AND SO DOES THE PERMISSION DATE (CUL-746). `permittedLaterFrom` decides the Why
    // column outright, and it was the one reason-bearing field left out of the key and
    // taken from whichever member happened to create the group (`panelWasRead` and
    // `attributionChecked` both carry comments saying first-member-wins was a defect;
    // this was the third instance and nobody had named it).
    //
    // The bias is structural, not incidental. A row's candidate `allowedFrom` dates are
    // those AFTER it, so the earliest member's candidate set is a superset of every
    // later member's — earliest-null implies all-null — and `exposures` is date-sorted
    // before grouping, so the group is always created by the member most likely to be
    // excused, and its excuse is then applied to the rest. The adversarial pass ran it:
    // a jerky permitted Jun 10–12, fed Jun 5 and again Jun 20, rendered ONE row reading
    // "Fed before it was permitted (allowed from Jun 10) ×2 | Jun 5 – Jun 20" — the
    // second feeding made eight days after the permission ENDED, excused by the first.
    //
    // It also re-opened round 5's "4 against 3" cross-check from the other side: page 1
    // now partitions on this field, so a group that straddles it made the two pages
...
function confounderRowHtml(
  r: ConfounderRow,
  markOffTrial: boolean,
  trialDerived: boolean,
  // ONE year decision for the whole table, from `permissionDatesNeedYear` (CUL-69/746).
  // It governs the Why column's permission date AND this row's own Dates column: a bare
  // span sitting one column from a year-stamped date inherits that year, which is the
  // defect one column over rather than a fix.
  withYear: boolean,
): string {
  const day = withYear ? fmtDayYear : fmtDay
  const span =
    r.firstDay && r.lastDay && r.firstDay !== r.lastDay
      ? `${h(day(r.firstDay))} &ndash; ${h(day(r.lastDay))}`
      : h(day(r.firstDay))
  ...
  // ONE PRECEDENCE RULE, SHARED WITH PAGE 1 (CUL-746). This column stated it correctly
  // and `exposureSentences` did not, for two rounds — which is what a rule duplicated
  // across two call sites looks like from inside the half that is right.
  const reason = exposureReasonOf(r)
...
function offDietAppendix(snap: ReportSnapshot): string {
  ...
  const withYear = snap.trial ? permissionDatesNeedYear(snap.trial) : false
  const rows = grouped.map((r) => confounderRowHtml(r, markOffTrial, trialDerived, withYear)).join('')
...
function appendixF(snap: ReportSnapshot): string {
  ...
  const photoDt = hasIncidentPhotos(snap)
    ...
    : ''
  // CUL-875 — gated exactly like `photoDt` above, for the same B-599 reason: a legend
  // entry describing a section the report does not carry is a dangling reference. What it
  // teaches is the one thing a vet cannot get from the appendix itself — that these are
...
const STYLE = `
  ...
  *{box-sizing:border-box;}
  ...
  /* The strip. Each cell is a fixed column so 28 days sit on one row at page width; the
     ...
     (No glyph characters in THIS comment: the stylesheet is interpolated into the
     document, so a CSS comment is shipped bytes — the R2-6 test, which forbids a triangle
     anywhere on a chartless report, catches exactly that and caught it here. And no
  ...
  /* An intervention STOP (R-14): a LONG-dashed rule with a heavy flat head. Two independent
     non-colour channels, because §5.8's bar is a photocopy and a dash pattern is the first thing
     a photocopy loses — the head still reads when the rhythm has merged, which is why the head
     and not the rhythm is the primary discriminator (the first cut used a fine dotted rule and
     a 1-bit fax simulation lost it entirely while the start's dashes survived). A flat cap,
     never a triangle: R2-6's ▲ was read as a data spike on the chart. */
  ...
  /* R-17/R-18's type pass from main — both axis labels at 10px --muted. Kept as main set them;
     R-14 only ever added the marker rules above. */
