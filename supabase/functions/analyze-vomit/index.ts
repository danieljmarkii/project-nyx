...
// and the write-back use the service role (trusted pipeline). Re-analysis
// never clobbers a human-edited field.
//
// B-247 PR 2: the incident-agnostic pipeline (auth → gate → context → image
// → vision → floor → write-back) lives in _shared/incident-analysis.ts and
// is driven by VOMIT_DESCRIPTOR below. This file keeps everything
// vomit-SPECIFIC: the clinical thresholds, the enum vocabularies, the tool
// schema + system prompt, the contextual-flag computation, the floor rule,
// and the owner-facing copy. The exported helpers keep their original
// signatures (as thin wrappers over the shared mechanisms) so the existing
// test suite pins that this refactor changed no behavior.
...
// Incident-agnostic helpers that moved to _shared (B-247 PR 2), re-exported so
// the test suite — and any future importer — keeps addressing them from here.
export {
...
export type { AnalysisWriteBack, FunctionCaps, GateState } from '../_shared/incident-analysis.ts'
...
export function parseAnalysisToolResult(response: ClaudeResponse): VomitAnalysis | null {
  const input = findToolUseInput(response, 'analyze_vomit') as VomitToolInput | null
...
// The escalation floor, vomit rule-set: ANY visual flag (blood, suspected
// foreign material) escalates. The mechanism — contextual flags force
// worth_a_call, the model can never downgrade, no reassuring verdict exists —
// is framework-owned (applyIncidentEscalationFloor). Wrapper keeps the
// original tested signature.
export function applyEscalationFloor(params: {
  ...
  return applyIncidentEscalationFloor({
    ...
    appearsToShowIncident: params.appearsToShowVomit,
    hasPhoto: params.hasPhoto,
    visualEscalation: params.visualFlags.length > 0,
...
// Vomit's owner-facing copy, selected in the framework-owned B-060 order
// (selectIncidentReadText). Every template here is covered by the Pattern-8
// reassurance-regex test in index.test.ts.
const VOMIT_READ_TEMPLATES: IncidentReadTemplates = {
  contextual: (petName, flags) => buildContextualReadText(petName, flags as ContextualFlag[]),
  photoUnreadable: buildPhotoUnreadableReadText,
  visualEscalationFallback: buildVisualFlagReadText,
...
// reassurance-on-absence (monitor) path — is a deterministic template. Wrapper over
// the framework-owned selection order, with vomit's templates.
export function selectReadText(params: {
  ...
  return selectIncidentReadText(params, VOMIT_READ_TEMPLATES)
}

// ── Write-back: the server half of the never-clobber guard (B-028) ─────────────
// The mechanism (edited row → update read fields only; otherwise full upsert)
// is framework-owned (buildIncidentWriteBack); this wrapper supplies vomit's
// structured column map and keeps the original tested signature.

function vomitStructuredColumns(analysis: VomitAnalysis | null): Record<string, unknown> {
  return {
...
export function buildAnalysisWriteBack(params: {
  ...
  return buildIncidentWriteBack({
    humanEdited: params.humanEdited,
    eventId: params.eventId,
    petId: params.petId,
    incidentType: 'vomit',
    analysisPayload: params.analysis,
    confidence: params.analysis?.confidence ?? null,
    structuredValues: vomitStructuredColumns(params.analysis),
...
// ── Cap + flag gate keys (Monetization Track 2, T2-3 / B-329 + B-001) ─────────
// docs/monetization-and-throttling-requirements.md §4–§5. The gate mechanics
// (config/usage readers + the pure decision) are framework-owned; these are
// analyze-vomit's keys. Free caps (§4.4): daily 10 / monthly 200 — DELIBERATELY
// identical across tiers (D-M2): the cap gates the descriptive read only; the
// deterministic escalation floor fires regardless (§5.4). Overridable via
// app_config.ai_caps.
const CAPS: FunctionCaps = { daily: 10, monthly: 200 }
const FUNCTION_KEY = 'analyze_vomit'
const FLAG_KEY = 'ai_vomit_read_enabled'

// ── The descriptor (B-247 PR 2, D2) ────────────────────────────────────────────
// Everything vomit-specific the shared pipeline needs, in one place. The
// pipeline (auth → gate → context → image → vision → floor → write-back) lives
// in _shared/incident-analysis.ts; adding the next incident type (analyze-stool)
// means writing a sibling of this object, not copying a handler.

const VOMIT_DESCRIPTOR: IncidentDescriptor<VomitAnalysis> = {
  functionName: 'analyze-vomit',
  eventTypes: ['vomit'],
  wrongTypeError: 'Event is not a vomit event',
  ...
  appearsToShowIncident: (a) => a.appears_to_show_vomit,
  // Vomit's floor rule: ANY visual flag escalates (blood, suspected foreign
  // material — the model only raises escalation-worthy flags for vomit).
  escalatesOnVisual: (a) => a.visual_flags.length > 0,
  ...
  readTemplates: VOMIT_READ_TEMPLATES,
  structuredColumns: vomitStructuredColumns,
}
...
if (import.meta.main) {
  Deno.serve((req) => runIncidentAnalysis(VOMIT_DESCRIPTOR, req))
}
