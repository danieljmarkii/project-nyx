...
export function parseAnalysisToolResult(response: ClaudeResponse): StoolAnalysis | null {
  ...
  return {
    ...
    // that the model writes it only on 'yes' — CUL-240 (B-042, on the vomit sibling) found
    // it populated on 'unsure' too. So a consumer must NOT infer foreign_material_present
    // === 'yes' from a non-null note (gate on the enum, never the note), and must NOT
    // surface the RAW note on a non-worth_a_call card (clinical-guardrails Pattern 10 —
    // model free text rides only the self-escalated path). Stool's 'unsure' detail render
    // is CUL-542.
