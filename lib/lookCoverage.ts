...
//     owner on day four.
//   • AFTER A WITHHELD PERIOD IT DOES NOT RENDER UNTIL THE WINDOW HAS MOVED PAST IT —
//     and NEVER as the saturation form, which the fourth pass showed would make one
//     sentence mean both "you answered every day" and "your cat was ill". On a chronic
//     pet the footer may be absent for long stretches, and absence carries no achievement
//     reading.
...
export function lookCoverage(record: readonly LookDayRow[], input: LookCoverageInput): LookCoverage {
  ...
  if (input.lastWithheldDay !== null) {
    const markIndex = localDayIndexOf(input.lastWithheldDay);
    ...
    if (markIndex === null || markIndex >= firstIndex) {
