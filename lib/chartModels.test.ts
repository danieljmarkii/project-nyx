...
describe('weeklyBuckets — Sunday-start weeks ending with a chosen week', () => {
  ...
  it('the partial week: days ahead carry no tick, and it says "N days so far"', () => {
    ...
    expect(daysSoFarLabel({ ...w, daysSoFar: 1 })).toBe('1 day so far');
  });
  ...
  it('the mark sits at its day\'s fractional slot, dated; outside the weeks it is null', () => {
    const m = weeklyBuckets({
      ...base,
      episodeDays: [],
      loggedDays: [],
    ...
    expect(m.mark).toEqual({ slot: 6 / 7, label: 'trial · Sep 12', day: shift(SUNDAY, -8) });
    const out = weeklyBuckets({ ...base, episodeDays: [], loggedDays: [], mark: { day: shift(SUNDAY, -40), label: 'x' } });
...
describe('laneDots — the shipped panel\'s geometry, per lane', () => {
  it('positions equal patternsTimingPos for the same minutes, ascending', () => {
    ...
    const lane = laneDots({ label: 'In the trial', timedMinutes: minutes, total: 6 });
  ...
  it('the three bucket counts partition the timed episodes via classifyGapMinutes', () => {
    ...
    const lane = laneDots({ label: 'x', timedMinutes: minutes, total: 8 });
  ...
  it('"N timed of M" and the untimed count come from the same two numbers', () => {
    const lane = laneDots({ label: 'Before the trial', timedMinutes: Array(13).fill(20), total: 19 });
  ...
  it('Dr. Chen: seven of seven in-trial under 30 minutes with fourteen untimed still says fourteen are missing', () => {
    const lane = laneDots({ label: 'In the trial', timedMinutes: [3, 5, 9, 12, 16, 21, 24], total: 21 });
  ...
  it('a total below the timed count is raised, never a negative disclosure', () => {
    const lane = laneDots({ label: 'x', timedMinutes: [1, 2, 3], total: 1 });
    expect(lane.total).toBe(3);
    expect(lane.untimedCount).toBe(0);
  });

  it('the untimed line is always a string — at zero it says so', () => {
    const a = laneDots({ label: 'a', timedMinutes: [10], total: 1 });
    ...
    const one = laneDots({ label: 'b', timedMinutes: [], total: 1 });
  ...
  it('drops a non-finite minute rather than plotting NaN', () => {
    const lane = laneDots({ label: 'x', timedMinutes: [Number.NaN, 12], total: 2 });
