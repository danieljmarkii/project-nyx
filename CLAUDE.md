...
| **B-182 chronicity lane (detector ⑦) — build greenlight + 2 routed decisions.** Build-ready spec written 2026-06-26 (`docs/nyx-chronicity-signal-requirements.md`): deterministic `symptom_chronicity` safety lane — fires on span + sustained-burden + still-ongoing (orthogonal to ④'s week-over-week delta), with the load-bearing ⑦→③ suppression valve (shared `isChronic`) closing the "improving tail of a chronic course" false-reassurance the council named. Routed to PM/Dr. Chen (spec §9): **(D1)** same-symptom ④↔⑦ interaction — recommend ⑦-suppresses-④ with firm-tier inheritance vs. co-fire (clinically load-bearing composition call); **(D2)** ratify the v1 floors (minSpan 21d / minEpisodes 4 / minActiveWeeks 3 / ongoingRecency 14d / firmSpan 42d / window 56d) — the build's noise property test is the empirical check before they lock; **(D3)** greenlight to build the 3 gated PRs. | Step 10 evolution; B-182 | Open — spec ready 2026-06-26; PM greenlight + D1/D2 calls. |
