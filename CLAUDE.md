...
|---|---|---|
| **Elevate a per-incident red flag (blood / suspected foreign material) to a high-visibility Home Signal?** Surfaced by the PM's first real vet-report send (2026-07-13). Today `analyze-vomit` computes `worth_a_call` + `visual_flags` on the event DETAIL screen only; `generate-signal/detection.ts` reads events/meals/meds but **not** `event_ai_analysis`, so a per-incident red flag never reaches Home — the primary intelligence surface where "safety insights always lead" (Principle 3). The invariant PERMITS it: n=1 may escalate on the PRESENCE of a red flag (never reassure on absence), and a safety card is escalation, not reassurance (**Dr. Chen + Data lean: surface it** — a worried owner shouldn't have to open a detail screen to learn the AI saw possible blood). **Genuine persona conflict:** the same session produced TWO false-positive flags (toy-in-frame; deli-meat seasoning) — **Designer + Jordan/Sam: a high-vis state that cries wolf twice trains owners to ignore the safety register** (Principle 4, trust). Reframe first: Nyx has no red "danger" state — safety cards are firm-but-calm — so "danger Signal" ≈ the existing firm safety register, led at top. Sub-decisions if yes: which classes (blood vs the more FP-prone foreign-material); **override-awareness (B-339 is a hard prereq** — a cleared flag must clear the card); single-incident vs require corroboration; calm-escalation copy (never a klaxon). Reads a NEW source into the Signal engine → `adversarial-reviewer` mandatory. | Step 10 evolution / Signal safety surface | **Open — surfaced 2026-07-13; PM call (persona conflict, do not resolve silently).** |
