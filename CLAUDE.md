...
- **Current Phase:** Step 9 — Vet report (resuming — B-010 capture + display surfaces COMPLETE). **B-010 PR C merged (#51):** event-detail confidence display, `edit-event` editable witnessed/found control (non-meal events; meals stay witnessed), History read-only marker — all via the shared `describeOccurredAt` formatter (QA notes 1 & 2 closed; on-device QA passed). **Next: resume Step 9 vet report** — render estimated/window times in the report (B-010 step 5), but **first resolve the blocking PDF-library open question** (`pdf-lib` vs `puppeteer` vs `react-pdf`) — it gates all PDF work. Then Step 10 correlation weighting (B-010 step 6).
