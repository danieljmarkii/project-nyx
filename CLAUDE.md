...
**Version:** 1.19 | Last Updated: May 2026
...
- **Blocking Open Questions:** PDF rendering library for Step 9 (`pdf-lib` vs `puppeteer` vs `react-pdf`); TestFlight workflow specifics (see Open Questions table)
- **Open PM Action Items:**
  - [ ] **Start Apple Developer enrollment ($99/yr, 1–3 day approval).** Blocking — no working sharing mechanism exists until this lands. See v1.20 retraction in Version History.
  - [ ] Once Apple Developer is approved, kick off a session for the TestFlight + EAS Update setup (one-time `eas build --profile preview`, TestFlight Internal Testing invite for wife, then `eas update --branch preview` works as designed)
  - [ ] Do NOT run `eas update` until the TestFlight build is in place. It's a no-op against Expo Go and just burns time / creates false sense of progress.
- **Runtime in use:** **None working.** v1.18's "EAS Update + Expo Go" was retracted in v1.20 — Expo Go ignores any bundle with `runtimeVersion` set. PM is using a cached bundle on his own phone (good enough for personal daily use; upload sync to Supabase works fine). Wife-onboarding is blocked until TestFlight path is set up.
- **Last session:** v1.20 (Runtime retraction. Day-after-v1.19 attempt to validate the remote→local sync fix on wife's phone exposed that the v1.18 daily-driver runtime never actually worked — Expo Go does not consume EAS Update bundles when `runtimeVersion` is set in app.json. Expo's own docs are explicit on this; we picked Runtime A on a misread. PM's phone was loading a cached Metro bundle the whole time, which is why he never noticed. Session ended with a clean pivot: kill the Expo Go path, target TestFlight + EAS Update as the real Runtime A once Apple Developer enrollment lands. The v1.19 code (PR #25) is correct but unvalidated end-to-end on a second device; PR stays open as draft until the dev build / TestFlight path can verify it.)
...
- Building "sync" as upload-only and calling Step 8 done — local-first apps need bidirectional sync. Without a remote→local pass, a second device signed into the same account shows an empty timeline because its local SQLite starts empty and nothing populates it. Every new sync surface must include both directions, a watermark (e.g. `MAX(updated_at)`), and an `ON CONFLICT … WHERE synced = 1` clause so pending local edits are not clobbered by an incoming server row.
- Setting `runtimeVersion` in `app.json` while expecting Expo Go to consume the bundle — Expo Go silently ignores any EAS Update bundle that has a custom `runtimeVersion`. The publish succeeds, the phone "loads" the project (often serving a stale cached bundle), and nothing surfaces an error. `eas update` distribution requires either a development build (`eas build --profile development`) or a TestFlight build (`eas build --profile preview --auto-submit`). Do NOT rely on Expo Go for sharing or OTA distribution. See v1.20 retraction in Version History; docs at https://docs.expo.dev/develop/development-builds/expo-go-to-dev-build/.
...
- Fresh device signs into an existing account (e.g. partner installs Expo Go on their phone): on first launch the local SQLite is empty. The timeline must populate from Supabase, not just show a blank "no events yet" state. Verify on every change to `lib/sync.ts` or the auth/pet bootstrap that `downloadRemoteData(petId)` still fires after the first upload pass and on every `activePet` change.
...
| TestFlight + EAS Update workflow specifics. Once Apple Developer enrollment lands, decide: (a) `eas build --profile preview` and submit to TestFlight Internal Testing (max 100 testers, no Apple review for internal), or (b) `eas build --profile development` and distribute via direct install (ad-hoc provisioning, ~2 devices easily, more painful for additional testers). Also: bundle identifier (`com.danieljmarkii.nyx`?), code signing setup, App Store Connect app record creation, push notification entitlements (defer or include now?). | Wife-onboarding; all post-MVP distribution | Open — kick off in the Apple-Dev-approved session |
...
| Step 8 (Offline sync) was marked complete but only the upload direction was built — a second device signed into the same account showed an empty timeline because no code path pulled events down from Supabase. Surfaced when PM tried to onboard his wife's phone via Expo Go. | Step 8 | **Resolved May 2026 (v1.19).** Added `downloadRemoteData(petId)` in `lib/sync.ts` — `MAX(updated_at)` watermark, upsert-by-id with `WHERE synced = 1` to preserve pending local edits, meals child-row pull keyed on the events just fetched. Wired into `useSync` after the upload pass and on `activePet` change. Attachments + vet_visits remote→local deferred to B-010. |
...
|---|---|---|
| v1.18 | May 2026 | **Daily-driver runtime change — RETRACTED in v1.20.** PM wanted to actually live in the MVP on their phone without keeping a Codespace + Metro tunnel alive. Set up EAS Update + Expo Go as the daily-driver runtime — `eas.json` added with `development` / `preview` / `production` build profiles and matching update channels; `preview` channel is what `eas update` publishes to. Daily flow: `eas update --branch preview --message "..."` from Codespace → fully close + reopen Expo Go on phone → Nyx loads the new bundle. CLAUDE.md Dev Handoff split into **Runtime A** (default — `eas update` + Expo Go, no Apple Developer needed) and **Runtime B** (active dev — Metro + tunnel, only when hot reload matters). Secrets Register gained rows for `EXPO_TOKEN` (optional, for CI automation) and Apple Developer account (not yet enrolled — blocks graduation to TestFlight / standalone iOS builds). Status block updated with three PM Action Items: one-time `eas init` / `eas update:configure` setup, smoke test on phone after first publish, and starting Apple Developer enrollment in parallel. **(See v1.20 — Expo Go silently ignores EAS Update bundles when `runtimeVersion` is set. This entire runtime was a misconfiguration.)** |
| v1.19 | May 2026 | **Step 8 hotfix — remote→local sync.** PM tried to share the app with his wife via Expo Go on her phone; she signed into his Supabase account and the timeline rendered empty. Investigation confirmed the sync path was upload-only: `app/(tabs)/history.tsx`, `hooks/useTrend.ts`, and `hooks/useEvents.ts` all read local SQLite, and `lib/sync.ts` only pushed local→remote — nothing ever pulled events down. Step 8 was implicitly only half-built; flagged as an engineering anti-pattern and marked as a Step 8 resolution in the Open Questions table. Added `downloadRemoteData(petId)` in `lib/sync.ts` (`MAX(updated_at)` watermark per pet, upsert-by-id, `WHERE synced = 1` guard to preserve pending local edits, meals child-row pull keyed on the events fetched). Wired into `useSync` after the upload pass, with `activePet?.id` in the dep array so the download fires once the pet resolves. New QA edge case added for fresh second-device sign-in. Pure code fix — no schema migration. Attachments + vet_visits remote→local sync deferred to backlog item B-010; wiring up the Jest harness so the `.test.ts` files actually run deferred to B-011. |
| v1.20 | May 2026 | **Runtime retraction — v1.18 was wrong.** Attempting to validate the v1.19 sync fix on the wife's phone surfaced that the v1.18 daily-driver runtime ("EAS Update + Expo Go") never actually worked. Expo Go silently ignores any EAS Update bundle that has `runtimeVersion` set in app.json, which ours does (`policy: "appVersion"`). PM's phone was loading a cached Metro bundle the whole time, which is why nobody noticed until a fresh second device with no cache showed up. Documented behavior at https://docs.expo.dev/develop/development-builds/expo-go-to-dev-build/ — should have caught this in v1.18, didn't. Two hours of debugging burned today chasing a problem caused by the misconfiguration. PM made the call to pivot to TestFlight + EAS Update as the real Runtime A once Apple Developer enrollment lands (already a PM Action Item, now promoted to blocking). Added Engineer anti-pattern: "Setting `runtimeVersion` while expecting Expo Go to consume the bundle." Added Open Question for TestFlight workflow specifics. Added backlog item B-012 (re-onboard wife once TestFlight is live). PR #25 (v1.19 code) stays open as draft — code is correct, just unvalidated end-to-end. No code changes this session beyond the Profile-tab Debug card (commit `2b5fd83`) which was built as part of the troubleshooting and is fine to keep — it'll be useful when TestFlight validation finally happens. v1.17 entry archived to `docs/CLAUDE-md-history.md` to keep the inline version-history table at 3 rows. |
