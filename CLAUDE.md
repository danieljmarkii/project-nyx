...
|---|---|---|
| **Activity-level capture — placement + whether to open the scope now.** PM-initiated discussion 2026-07-15 (`docs/activity-level-capture-discovery.md`). The team placed activity level as a **continuous/ambient** state → the **persistent-but-dismissible day-close card (#3)** is the right mechanism (FAB #1 = wrong class; push #2 = a Phase-2 amplifier gated on B-288 + the nudge-cap ruling). Two decisions remain. **(a) Placement — genuine persona conflict:** _Designer_ — a standing owner-*input* card is the first data-entry widget on Home, and Principle 3 says Home *curates insight, it does not collect*; argue for the Today zone / a day-close completion moment / the evening-reconciliation card, off the intelligence surface. _Dr. Chen / Data_ — activity **trend** is a first-line clinical marker and the sharpest fussy-vs-sick disambiguator (Sam); the day-close card is the only mechanism that fits the data's class. **(b) Sequencing** — open a net-new continuous-capture track now, or **ride B-288** (same class / same day-close ritual — team rec). Build-time (not PM-blocking): the anchored species-aware scale wired to never-reassure, and the `activity_check` schema shape (B-345). | Step 10+ capture scope; B-344 promotion | Open — surfaced 2026-07-15; PM placement + open-now-vs-ride-B-288 call needed. |
