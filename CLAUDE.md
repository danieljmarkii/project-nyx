...
| B-156 combo (med-with-food) — open build decisions before Phase B (the combo) can build (G1 already resolved — auto-dismiss + fail-safe): **G2** edit model (edit a linked treat+dose as one combo unit, or as two independent History instances?); **G3** cross-track order (land B-153/B-154 — the dose↔regimen link — before the combo so the shared `insertMedicationDose` path isn't built twice? team rec: yes); **G4** "ate-around-the-pill" (cat licks the vehicle clean but leaves the pill → treat reads "All", dose stays `given` → false-adherence: detect-and-prompt, or document as a known limit?); plus **promotion** (move Slice B `how_given` (PRs A1–A3, ungated) to active build now, or hold behind the `Now` med items?). Detail + full PR plan: `docs/medication-food-combo-investigation.md` §10. | B-156 Phase B (combo) build; Slice B promotion | Open |
### Resolved
