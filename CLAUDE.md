...
- **Current Phase:** Step 9 — Vet report (paused — B-010 in progress). **Capture COMPLETE:** schema migration #45 + quick-log capture UI #48 both merged to `main` (Direction 2 witnessed/found control; migration applied to live DB). **Next: B-010 PR C** — event-detail timestamp+confidence display, `edit-event` editable control, History read-only marker (QA notes 1 & 2). Then vet-report rendering resumes Step 9.
...
- **Last session:** B-010 capture shipped end-to-end. Merged schema migration #45 (`occurred_at_confidence` enum + nullable `occurred_at_earliest`/`occurred_at_latest`; **NULL legacy backfill** — PM hand-populates the small set; applied to live DB) and quick-log capture UI #48 (Direction 2 "Saw it / Found it" control on the simple-event step). Decisions: **Direction 2** (explicit pills, witnessed preset, optimizes the ~65% found majority while keeping witnessed one-tap); **windowed `occurred_at` = latest edge, not midpoint** (a real entered value, not invented — window fields stay the source of truth, and surfaces must render the window, never the bare point, when confidence ≠ witnessed); confidence is set by the affordance, never inferred from EXIF. PM QA (on device) passed notes 3 & 4 after fixes: 3-mode found selector so all modes are reachable (no dead-end), estimated-point kept separate from the witnessed point, dark-mode picker fix. Filed **B-025** (FAB quick-symptom chips bypass the control — resolve with B-007 FAB revamp) and **B-026** (no jest runner in repo). **Next: PR C** — detail-screen display + `edit-event` editable control + History marker (QA notes 1 & 2: confidence isn't editable after logging, and isn't shown on detail screens).
