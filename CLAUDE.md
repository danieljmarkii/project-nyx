...
| **Capture ALL proteins in a food, not just `primary_protein`?** Surfaced by the PM during B-332 (2026-07-15). The food model carries a SINGLE protein and the flagship diet correlation keys off it. **Persona split (real, not silently resolved):** Dr. Chen/advisors lean YES — for the elimination/novel-protein trial (the wedge), hidden *secondary* proteins are the top reason a home trial silently fails (a "duck" food that also lists chicken by-product meal), so primary-only misses the exposure that matters most. Data Scientist counters that a multi-protein exposure model is harder to attribute (collinear co-occurring proteins, bloated exposure sets, lower per-protein n) — more *sensitivity to hidden exposure*, less *culprit attribution*. Done right it's ONE cross-cutting change (extraction emits all → schema holds them → engine keys on set-membership → vet report renders them), likely absorbing B-048; full ingredient text is already captured (`ingredients_notes`), just unstructured. **Deliberately NOT in B-332** (single-protein parity-with-extraction, ships now). Build tracked as **B-351**. | Step 10 correlation quality; vet-report protein exposure; B-351 | **Open — narrowed 2026-07-19: design session produced a build-ready DRAFT spec (`docs/nyx-multi-protein-requirements.md` v0.1) with provisional decisions AWAITING PM/Dr. Chen ratification (the session ran non-interactively, so D1–D6 are recommend-and-proceed, not yet ruled).** The spec argues the sensitivity-vs-attribution split is a **false binary**: capture wins the *exposure* job outright (you can't detect a contaminant you never recorded), while *attribution* stays honest via **collinearity-aware joint candidates** ("chicken and/or duck — can't yet separate") + the existing staple-washout (B-070) + effective-n floors, so the Bonferroni family is sized by *discriminating clusters*, not raw protein count. Provisional rulings (§10): **D1** capture-all, phased; **D2** pull the *deterministic trial-contaminant check* forward as Phase A (a "duck" trial food that also lists chicken flags at capture — near-zero statistical risk, wedge-centred); **D3** absorb B-048 (the multi-protein set *is* the structured `ingredients_notes`); **D4** `food_items.proteins TEXT[]` (ordered, `[0]`=derived `primary_protein`, additive/back-compat — join table rejected, jsonb deferred); **D5** joint-candidate attribution copy; **D6** no `diet_trials.excluded_proteins` in v1. 6-PR phased plan (§11): Phase A capture+contaminant (schema → extraction → multi-select `ProteinPicker` → trial flag), Phase B set-membership correlation (`adversarial-reviewer` MANDATORY, deploy-gated on the client renderer per the B-182 lesson) → vet report. Needs the D1 sensitivity-vs-attribution ruling (+ Dr. Chen on D5) before the spec locks. |
