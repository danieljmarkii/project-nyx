...
- **ATTACHING a PR to an issue is a CLOSING action, not a "related work" link (CUL-803, 2026-09-05).** The integration moves an issue to `Done` when an attached PR merges, **whether or not the PR body used a magic word** — attaching is enough. So attach a PR to an issue only when merging it genuinely finishes that issue. CUL-803's PR was attached to CUL-660 as related work, and merging it closed CUL-660 — an issue with an open decision brief, its own draft PR and nothing built. To point at related work without closing anything, say so in an issue **comment** and leave the attachment off.
