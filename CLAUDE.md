...
**Version:** 1.26 | Last Updated: 2026-07-25
...
|---|---|---|
| **Diet trial (B-417) — six open rulings after the 2026-07-25 requirements review.** Full tee-up with evidence, options and dissent: **`docs/diet-trial-requirements-review-2026-07.md`** §3–§4. **G1/D3 — the allowed-food set vs. B-351's D6.** Team rec: **ratify — unanimous across nine lenses and now empirical.** Applied to the live account the *shipped* off-diet definition would report **~530 exposures across 645 feedings** (82% of logged feedings are treats); the explicit set is what makes an exposure count mean anything. Plus label unreliability (33–83% mislabeling *in the products marketed for these trials*, so an explicit set is a record of intent that survives a wrong panel while derivation inherits its errors) and owner recall (83.1% at two instructions → 28.6% at four). **G2 — reframed, and this is the important one.** The question "below what coverage may the app say *no off-diet foods logged*?" is malformed: it presumes the sentence becomes sayable above a floor, and four code-grounded proofs say it never is (a refused-diet trial scores 100% coverage / 0 exposures; treat-only days clear any floor; the day-granular metric saturates for the once-a-day logger; the oral-medication channel is invisible). **Team rec: delete the negative claim from the product at every coverage; render a positive statement about the *record* with its qualifier inline.** No mature elimination app in any species writes that sentence. **G3 — numbers stand, semantics change.** Skin 56d **is** the >90% band (not "the low end" — correct §0.3); GI 28d is an *assessment* point against an ACVIM-2026 recommendation to continue ≥12 weeks, so the milestone must never read as permission to stop. Key defaults on **species × indication**. **§0.2 sequencing** — team rec option **(c)**, plus a correction: the B-351 dependency is **soft, not hard** (D10 is presence-only, so the protein arm can only *add* a verdict), which unblocks PRs 4 and 5. **Conflicts C1–C6** — PR 5's half-path; trial-diet self-contamination; the oral-route lane; how much §7 may grow (a genuine Principle-6 layout conflict); an owner-scored severity value (amendment against D1); and what the owner is told at "Start a trial" (T&S: this is Nyx's first record that is a judgment about a *person*). | B-417 PR 1 (migration 040) onward | Open — **spec v0.95, 2026-07-25.** PR 1 is the gate and carries ~9 schema decisions, all free at zero live rows. |
