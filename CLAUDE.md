...
- **Subagent** (`.claude/agents/`) — a bounded, isolated-context review that returns a verdict: `adversarial-reviewer` (falsification pass on clinical/statistical logic), `code-reviewer` (diff review), `rls-privacy-reviewer` (access-control red-team: share tokens, service-role queries, RLS, Storage, deletion/export — the adversarial-reviewer's sibling for boundaries instead of statistics), and `vet-report-cold-read` (Dr. Chen reading the *rendered* report cold, once Step 9 renders one). Isolation is a *feature* for adversarial review — the reviewer is not anchored by the build conversation's optimism (and for the cold read, it mirrors how the artifact is actually consumed: by a vet with zero build context).
