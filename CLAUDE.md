...
- **Current Phase:** Step 9 — Vet report (paused — B-010 event timestamp uncertainty is the Step 9 prerequisite. Schema model now DECIDED: Option C (`occurred_at_confidence` enum + window fields). Remaining B-010 work: quick-log UX sketch → schema migration PR → vet-report rendering, then resume Step 9)
...
- **Last session:** B-010 schema-model decision. Reviewed the event-timestamp-uncertainty research brief, ran the personas through A/B/C, and sketched the quick-log UX for B vs C. **PM decided Option C** (`occurred_at_confidence` enum `witnessed`/`estimated`/`window` + nullable `occurred_at_earliest`/`occurred_at_latest`, `occurred_at` kept as derived point). Rationale: C is the only option that honestly represents both real triggering incidents (the ~4am point-estimate AND the 2–4pm window); it renders down to a simple point view while B can never render up; and the ~65% discovered majority means the found path must be both cheap and honest, which only C delivers. Confidence derived from the affordance the user touches (never a quiz) keeps witnessed one-tap. No code yet — next is the quick-log UX sketch (one-tap "Found it" default for discovery-prone events), then the schema migration PR.
