...
- **Open PM Action Items:**
  ...
  - [ ] After publishing this v1.19 fix via `eas update`, fully close + reopen Expo Go on both phones (yours + your wife's) and confirm timeline now populates on both — see Manual QA Script in the Dev Handoff
...
- **Last session:** v1.19 (Step 8 hotfix — remote→local sync. Wife's phone showed an empty timeline after signing into PM's existing account because `lib/sync.ts` was upload-only; Step 8 was implicitly only half-built. Added `downloadRemoteData(petId)` in `lib/sync.ts` with a `MAX(updated_at)` watermark, an upsert-by-id that preserves pending local edits via `WHERE synced=1`, and a meals child-row pull keyed on the events just fetched. Wired into `useSync` after the upload pass and on `activePet` change. Pure code fix — no schema migration. Attachments + vet_visits remote→local deferred to B-010.)
