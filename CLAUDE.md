...
**Version:** 1.33 | Last Updated: 2026-09-09
...
| `docs/nyx-incident-screen-requirements.md` | Any session on the incident-screen track (CUL-800: CUL-802 the route · CUL-803 the screen · CUL-804 the arrival; CUL-801 the trigger race) or touching what happens after a vomit / stool is logged, `app/event/[id].tsx`, `VomitAnalysisSection` / `StoolAnalysisSection`, the named card's placement, or any surface where a per-incident AI read appears. 🌱 **v1.0 BUILD-READY** (D1–D4 PM-ruled 2026-09-05): a photographed vomit / stool lands on its record (D1) and only those (D2 — Principle 1 survives by scope, never by tie-break); the photo stays the hero (D3, "show it to a vet") and the viewer gains a time caption; the read arrives with the Signal fold's rail-led physics (D4), same on every verdict, no haptic. G5: a screen never shows a row that is no longer in the record — Undo from over the record dismisses it first. Design authority `docs/culprit-incident-screen-mockups.html` round 2. |
...
- **A shared choreography lives in `components/motion/`, and a trigger switches on the FACT, never the presentation (CUL-804):** a second surface that needs an existing motion lifts the module rather than restating its constants — `foldMotion.ts` (the Signal fold) and `arrivalMotion.ts` (the per-incident read's arrival) share one vocabulary and the two-engine split (`LayoutAnimation` for geometry, native-driver `Animated` for everything else; a layout keyframe re-commits a view's props, so an animated node leaves the flow for that commit). Prefer a sibling hook to a mode flag when the two defer different things. And two states that render identically are not the same state: the arrival's trigger reads `working` (the server was asked), never "the pending box is on screen" (which is also true while the local row is being read). _(full account: `docs/engineering-lessons.md` §C-30)_
...
- **Haptics (CUL-604, CUL-601; polish spec §5.6):** never import `expo-haptics` at a call site — use the moment-named verbs in `lib/haptics.ts` (one verb per moment, the export list pinned by `lib/haptics.test.ts`; a symptom commit is a soft impact, never a success). Silence on safety is structural: `guards/haptics.test.ts` fails the build if a safety component imports the module (exemption `// haptics-guard-ok: <reason>`); the one exemption, `SignalZone` (CUL-601), is paid for with a gate and a test, never by moving the call to an unscanned file. **Extracting a safety RENDER out of a scanned file owes an `ALWAYS_SCANNED` entry for what came out (CUL-803)** — the scan set is derived from source markers, so a refactor that moves the surface painting a `worth_a_call` into a new file leaves the guard green over it; prove the new entry by mutation. Every verb is fire-and-forget; a commit haptic fires at the moment store's reveal, not at the call site. _(full account: `docs/engineering-lessons.md` §C-16)_
...
- **Every commit path routes through its completion card (CUL-613), and every guard is proven by MUTATION (CUL-621, CUL-712):** an `insertMeal(` / `insertMedicationDose(` call site must reach `showMeal` / `showMedication`; `guards/completionCard.test.ts` enforces it (exemption `// completion-card-ok: <reason>`; `MedStrip.tsx` holds the one). Run a new guard against the tree it was written for before trusting it, and prove it by breaking the source rather than by reading the test; split a new test by required direction (a guard reds pre-fix, a refactor-safety test is green before and after). A detector fixture lives OUTSIDE the scanned tree via `guards/fixtureRoot.ts` (`createFixtureRoot`, a REQUIRED `root` parameter, teardown guarded by provenance and containment), and the live scan is proven by dropping a real violation into the real tree. **Reaching past that helper now fails the build (CUL-714):** `guards/fixtureDiscipline.test.ts` reds on a filesystem WRITE in `guards/` whose destination is repo-anchored (`REPO_ROOT` / `ROOT` / `__dirname` / `process.cwd()`), following one LOCAL hop — a windowed one, since a file-wide hop binds a parameter to a same-named `const` in another function; exemption `// fixture-root-ok: <reason>` within ten lines above (the two in `fixtureRoot.test.ts` are the probes that must be in-repo to drive the helper's own refusals). Blank strings as well as comments before any such scan: a guard's fixtures ARE the anti-pattern, held in a template literal, so a raw-source match reports the detector's own test data; blank them line-PRESERVINGLY, or every line number the exemption window reads is off by the height of the comments above it; and blank them in ONE left-to-right pass, never a chain of `.replace()`s — independent passes each read delimiters the other owns, so a `//` inside a string eats the rest of the line and an apostrophe in a `"…"` string pairs with the next stray quote, each silently swallowing a real violation. _(full account: `docs/engineering-lessons.md` §C-18)_
...
**A reaction round republishes as ONE proposal (2026-09-09, PM directive on the daily look: "sometimes we mix in elements from mock-a to mock-b and it gets confusing which is current state").** When the PM has reacted to a side-by-side round, the next round is not a second side-by-side: the retired directions **leave the page** (git keeps them; the page names the commit), every alternative that is still a real decision sits in a **labelled box** (*option, drawn for the decision, not the current proposal*) beside the frame it competes with, every current frame carries a *current* tag, and a **ledger at the top maps each reaction to what it ruled and what moved on the page** (a reaction that was a clarification, not a change, says so in the ledger rather than silently). First instance: `culprit-daily-look-mockups.html` round 2 (CUL-840).
