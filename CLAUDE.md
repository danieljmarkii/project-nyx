...
- `app_config` is the **sole** sanctioned globally-scoped table (app-wide product config). `food_items` + `medication_items` are per-account (PM-ratified 2026-07-16, B-354 — `docs/nyx-per-account-food-library-requirements.md`); until the re-scoping migration ships they are still global *in code*, but **no new code may assume the global catalog**. A future shared catalog returns only as a separate curated/canonical layer, never by un-scoping user rows. Every other new table includes `pet_id` and RLS.
...
|---|---|---|
| **Ask (B-228) — confirm D2 (the LLM data boundary) + rule D7 (the name).** The 2026-07-18 session produced the build-ready v2.0 spec + design-locked mocks (#394; supersedes the 2026-06-13 rev 1, absorbs B-088). PM ruled D1 (nested `ask_enabled`/`ask_general_enabled` flags, general seeded off), D3 (free teaser 3 convos/mo + Premium full), D4 (next main project), D5 (Home header pill; the entry never changes/badges when capped — Home carries no monetization state), D6 (answer anatomy). **Open: (1) D2** — recorded PROVISIONAL at **tier 2** (aggregates + whitelisted structured fields; never free-text notes, never photos; ratified-by-reaction at the mock review) and needs the explicit confirm T&S asked for, since it extends the prior findings+counts posture; **(2) D7** — name ("Ask" working label; rev 1's no-vet-implication ban stands, Dr. Chen + Lawyer). | Ask A3/A4 (the whitelist is built there); A5 (ships the label) | Open — D2 provisional 2026-07-18 (build may proceed; a tier-1 downgrade is a one-table change); D7 decide before A5 merges. |
