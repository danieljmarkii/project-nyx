...
- **Current Phase:** Step 9 — Vet report, **consciously interrupted (PM decision 2026-05-24) to build per-incident AI analysis — Vomit first (B-027, under parent B-013).** Dogfooding rationale: real incidents exist now; the vet report serves a not-yet-booked visit. Step 9 (vet report PDF, gated on the PDF-library open question) resumes after the vomit feature ships. Note: the vomit child is now the FIRST place we set the non-diagnostic clinical guardrail pattern — Step 10's AI Signal inherits it. **B-027 progress (2026-05-24):** clinical definition LOCKED (acceptance gate closed) — vomit prompt + structured field set + red-flag list + context-assembled escalation floor + no-reassure `recommendation` enum all decided with Dr. Chen; schema migration `013_event_ai_analysis.sql` written (PR #54, additive). Next is the Edge Function PR, then the detail-screen UI PR. _B-010 status:_ capture + display surfaces COMPLETE; **PR C merged (#51)** (event-detail confidence display, `edit-event` editable witnessed/found control, History read-only marker — all via shared `describeOccurredAt`). B-010 step 5 (vet-report estimated/window rendering) folds back in when Step 9 resumes.
...
- **Last session:** B-010 **PR C** shipped + merged (#51). Detail screen renders confidence-aware times (`~4:00 AM` estimated / `between 1:00 PM and 3:00 PM` / `found by 3:00 PM` window) with an honest, non-alarming "not witnessed" note, dropping the relative suffix + EXIF attribution when the time isn't exact; `edit-event` reuses `TimeConfidenceField` so **non-meal** events can be re-classified post-log (state reconstructed from stored fields on mount, meals forced witnessed); History rows carry a read-only `ESTIMATED`/`APPROXIMATE` marker. New shared **`describeOccurredAt`** (`lib/utils`) is the single source for witnessed/estimated/window phrasing — reuse it for the vet report and AI Signal. `lib/db`: `TimelineRow` + `getTimeline`/`getEventById`/`updateEvent` now carry the window fields; added `getEventTimeFields`. Unit tests for `describeOccurredAt` written following the `deriveOccurredAt` pattern but **unrunnable (B-026, no jest runner)**. On-device QA passed. **Next: resume Step 9 vet report** — confidence-aware time rendering (B-010 step 5), gated on the PDF-library decision.
