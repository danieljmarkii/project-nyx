...
3. **Write this session's record to `docs/sessions/`; touch `STATUS.md` only if a track boundary moved.** Two files with two different jobs — and in most sessions only the first one changes.
   ...
   **And if this session's merged code made a 🌱 living spec's statement false, correct the spec here too.** A doc-matches-code edit is **descriptive** — a factual correction, not a decision — so it ships in *this* PR and is never filed as a PM ratification request (CLAUDE.md § Documentation Update Protocol → Tier 2; CUL-924). Only a **prescriptive** edit — one changing what the code *should do next* — goes to the PM. 🧊 Frozen briefs are out of scope both ways: they take a dated §V row plus an inline ⚠ pointer, never an in-place rewrite.
