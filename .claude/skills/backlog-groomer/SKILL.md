...
2. **Fix stale in-flight issues.** An issue reading `In Progress` whose PR has merged is `Done`. An issue whose plan has since been executed is `Done` or partially done — if a defined slice remains, keep it open and note the remaining slice in a comment. Flag any issue that has sat in `In Progress` across multiple sessions with no linked PR: it's either genuinely blocked (say why in a comment), abandoned, or actually done and never moved. **Read its claim comment to tell which** (`/kickoff` step 0 — `**Claimed** — branch …`): it names the branch and the UTC time, so `git ls-remote origin <branch>` plus the PR list answers it. An old claim whose branch never appeared and whose issue has no PR is an **abandoned claim** — clear the issue back to `Todo` with a one-line comment naming the dead branch, so the next session can take it instead of reading it as taken. Status alone can't distinguish these: it names no branch.
