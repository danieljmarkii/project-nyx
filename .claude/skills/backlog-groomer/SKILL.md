...
4. **Enforce the row contract.** Every row needs: sequential `B-NNN` (never reused), Title, one-line _why_, Priority (`Now`/`Next`/`Later`), `Added` (ISO date), `Blocks` (or `—`), Status. Flag any row missing the _why_.
