...
1. **Reconcile Status against reality.** For each `Open` row, check whether it actually shipped: `git log --oneline | grep -i "B-0NN"`, the merged-PR list, the `docs/sessions/` records, and STATUS.md's current-phase claims. If it merged, rewrite Status to `Done — <date> (PR #N)` (or `Done — <date> (<session>)` for non-code closures like persona reviews). Never close without a resolving reference.
...
5. **De-duplicate — duplicate *IDs* first, then duplicate content.** IDs are machine-checkable, so check them rather than eyeballing: `grep -ohE '^\| B-[0-9]+ ' docs/backlog.md | sort | uniq -d` must print nothing. Any output means two sessions minted the same ID against a stale `main` (B-435) and every cross-reference to it is now ambiguous — resolve it with `/wrap` Step 4's rule (**the row that landed on `main` first keeps the ID**, the later arrival renumbers to the next free ID, cross-references updated *by attribution* rather than blind replace, old ID noted inline on the moved row). Then the content pass: if a row restates an existing item, prefer composing/cross-referencing (`composes with B-0NN`) over a new ID. Flag near-duplicates to the PM.
