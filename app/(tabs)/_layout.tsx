...
function FAB() {
  ...
  async function loadRecents(petId: string) {
    ...
    // Deduplicate by event_type + food_item_id, take first 3 unique
  ...
  async function handlePhotoLog() {
    ...
    if (status !== 'granted') {
      Alert.alert('Photo access needed', 'Allow photo access in Settings to attach images.');
      return;
    }
  ...
  function recentLabel(r: RecentEvent): string {
    ...
    const label = EVENT_TYPES[r.event_type]?.label ?? r.event_type;
